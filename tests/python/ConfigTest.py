...
"""
    ...
    def test_generate_local_id_for_color_space(self):
        # Test the generateLocalIDForColorSpace method.
        ...
        # lowered, spaces become '_', ',' becomes '.', and '!' becomes '*'.
        ...
        cfgNoName = OCIO.Config()
        cfgNoName.addColorSpace(cs)
        with self.assertRaises(OCIO.Exception):
            cfgNoName.generateLocalIDForColorSpace('sRGB Encoded,Space')

        # Color space that does not exist in the config.
        with self.assertRaises(OCIO.Exception):
            cfg.generateLocalIDForColorSpace('does_not_exist')

        # Renaming the config is reflected immediately (no stale caching).
        cfg.setName('Other Studio')
        self.assertEqual(cfg.generateLocalIDForColorSpace('sRGB Encoded,Space'),
                         'other_studio:local:srgb_encoded.space')

    def test_find_color_space_for_id(self):
        # Test the findColorSpaceForID method.

        cfg = OCIO.Config()
        cfg.setName('MyStudio!')

        cfg.addColorSpace(OCIO.ColorSpace(name='ACEScg', aliases=['lin_ap1']))
        cfg.addColorSpace(OCIO.ColorSpace(name='srgb'))
        cfg.addColorSpace(OCIO.ColorSpace(name='foo'))

        # A color space whose name and alias differ from their sanitized form by more than
        # just case, so they are only found via the local mode fall-back.
        cfg.addColorSpace(OCIO.ColorSpace(name='My Color Space!', aliases=['Fancy Alias']))

        # Mode 1: exact name and exact alias.
        self.assertEqual(cfg.findColorSpaceForID('acescg').getName(), 'ACEScg')
        self.assertEqual(cfg.findColorSpaceForID('lin_ap1').getName(), 'ACEScg')

        # Mode 2: one namespace, falls back to the base name.
        self.assertEqual(cfg.findColorSpaceForID('somestudio:acescg').getName(), 'ACEScg')
        ...
        self.assertIsNone(cfg.findColorSpaceForID('outer:inner:acescg'))

        # Mode 3 (local): outer namespace matches this config's sanitized name.
        self.assertEqual(cfg.findColorSpaceForID('mystudio*:local:acescg').getName(), 'ACEScg')

        # Mode 3 negative: outer namespace does not match the config's name.
        self.assertIsNone(cfg.findColorSpaceForID('othername:local:acescg'))

        # Mode 3 with names and aliases that must be sanitized to match the ID base.
        self.assertEqual(cfg.findColorSpaceForID('mystudio*:local:my_color_space*').getName(),
                          'My Color Space!')
        self.assertEqual(cfg.findColorSpaceForID('mystudio*:local:fancy_alias').getName(),
                          'My Color Space!')

        # The sanitized comparison is only used for the local mode fall-back.
        self.assertIsNone(cfg.findColorSpaceForID('my_color_space*'))
        self.assertIsNone(cfg.findColorSpaceForID('othername:local:my_color_space*'))

        # Empty inner namespace: "my-studio::srgb" strips to ":srgb", which must not match "srgb".
        self.assertIsNone(cfg.findColorSpaceForID('my-studio::srgb'))

        # Use of "local" as an ordinary namespace is not allowed, it may only be used as a keyword
        # for the local (mode 3) form of an ID.
        self.assertIsNone(cfg.findColorSpaceForID('local:foo'))

        # Garbage input: extra colons, empty string, and None all resolve to "not found"
        # without raising.
        self.assertIsNone(cfg.findColorSpaceForID('a:b:c:d'))
        self.assertIsNone(cfg.findColorSpaceForID(''))
        self.assertIsNone(cfg.findColorSpaceForID(None))

        # Round-trip: an ID generated by this config is resolved back to the same color space.
        generated = cfg.generateLocalIDForColorSpace('ACEScg')
        self.assertEqual(cfg.findColorSpaceForID(generated).getName(), 'ACEScg')

        # Round-trip for a color space whose name requires more than case sanitizing.
        generated = cfg.generateLocalIDForColorSpace('My Color Space!')
        self.assertEqual(generated, 'mystudio*:local:my_color_space*')
        self.assertEqual(cfg.findColorSpaceForID(generated).getName(), 'My Color Space!')

        # A config with no name never matches a local mode ID, not even one with an empty
        # outer namespace (such an ID could not have been generated by an unnamed config).
        cfgNoName = OCIO.Config()
        cfgNoName.addColorSpace(OCIO.ColorSpace(name='foo'))
        self.assertEqual(cfgNoName.findColorSpaceForID('foo').getName(), 'foo')
