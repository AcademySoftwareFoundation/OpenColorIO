...
class MixingHelpersTest(unittest.TestCase):
    ...
    def test_encoding(self):
        ...
        for param in [None, 1, OCIO.TRANSFORM_DIR_FORWARD]:
            with self.assertRaises(TypeError):
                mix.setSelectedMixingEncoding(param)  # type: ignore[arg-type]

        for param in [None, 'test']:
            with self.assertRaises(TypeError):
                mix.setSelectedMixingEncodingIdx(param)  # type: ignore[arg-type]
    ...
    def test_mixing_space(self):
        ...
        for param in [None, 1, OCIO.TRANSFORM_DIR_FORWARD]:
            with self.assertRaises(TypeError):
                mix.setSelectedMixingSpace(param)  # type: ignore[arg-type]

        for param in [None, 'test']:
            with self.assertRaises(TypeError):
                mix.setSelectedMixingSpaceIdx(param)  # type: ignore[arg-type]
    ...
    def test_get_processor(self):
        ...
        with self.assertRaises(TypeError):
            proc = mix.getProcessor('lin_1', None, 'VIEW_1')  # type: ignore[arg-type]

        with self.assertRaises(TypeError):
            proc = mix.getProcessor(workingSpaceName = 'lin_1', displayName = 666, viewName = 'VIEW_1')  # type: ignore[arg-type]

        with self.assertRaises(TypeError):
            proc = mix.getProcessor(workingSpaceName = 'lin_1', displayName = 'DISP_1',
                                    viewName = OCIO.TRANSFORM_TYPE_LOG)  # type: ignore[arg-type]

        with self.assertRaises(TypeError):
            proc = mix.getProcessor(workingSpaceName = 'lin_1', displayName = 'DISP_1')  # type: ignore[call-arg]
