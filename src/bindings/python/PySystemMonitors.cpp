...
{
    auto clsSystemMonitors = py::class_<PySystemMonitors>(m.attr("SystemMonitors"));

    auto clsMonitorIterator = py::class_<MonitorIterator>(clsSystemMonitors, "MonitorIterator");

    clsSystemMonitors
        .def(py::init<>(), DOC(SystemMonitors, SystemMonitors))

        .def("getMonitors", [](PySystemMonitors & self) { return MonitorIterator(self); });

    clsMonitorIterator
        .def("__len__", [](MonitorIterator & it) { return it.m_obj.getNumMonitors(); })
        .def(
            "__getitem__",
            [](MonitorIterator & it, int i) {
                // SystemMonitors provides index check with exception.
                return py::make_tuple(it.m_obj.getMonitorName(i), it.m_obj.getProfileFilepath(i));
            })
