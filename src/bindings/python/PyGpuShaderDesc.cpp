...
void bindPyGpuShaderDesc(py::module & m)
{
    ...
    auto clsGpuShaderDesc
        = py::class_<GpuShaderDesc, GpuShaderDescRcPtr, GpuShaderCreator>(m.attr("GpuShaderDesc"));

    auto clsUniformData = py::class_<GpuShaderDesc::UniformData>(clsGpuShaderDesc, "UniformData");

    auto clsUniformIterator = py::class_<UniformIterator>(clsGpuShaderDesc, "UniformIterator");

    auto clsTexture = py::class_<Texture>(clsGpuShaderDesc, "Texture");

    auto clsTextureIterator = py::class_<TextureIterator>(clsGpuShaderDesc, "TextureIterator");

    auto clsTexture3D = py::class_<Texture3D>(clsGpuShaderDesc, "Texture3D");

    auto clsTexture3DIterator
        = py::class_<Texture3DIterator>(clsGpuShaderDesc, "Texture3DIterator");
    ...
    clsGpuShaderDesc
        .def_static(
            ...
            [](GpuLanguage lang,
               ...
               const std::string & uid) {
                ...
                {
                    p->setFunctionName(functionName.c_str());
                }
                if (!pixelName.empty())
                {
                    p->setPixelName(pixelName.c_str());
                }
                if (!resourcePrefix.empty())
                {
                    p->setResourcePrefix(resourcePrefix.c_str());
                }
                if (!uid.empty())
                {
                    p->setUniqueID(uid.c_str());
                }
                return p;
            },
        ...
        .def("clone", &GpuShaderDesc::clone, DOC(GpuShaderDesc, clone))
        .def("getShaderText", &GpuShaderDesc::getShaderText, DOC(GpuShaderDesc, getShaderText))
        .def("getUniforms", [](GpuShaderDescRcPtr & self) { return UniformIterator(self); })
        .def(
            "getUniformBufferSize",
            &GpuShaderDesc::getUniformBufferSize,
            DOC(GpuShaderDesc, getUniformBufferSize))

        // 1D lut related methods
        .def(
            "addTexture",
            [](GpuShaderDescRcPtr & self,
        ...
        .def("getTextures", [](GpuShaderDescRcPtr & self) { return TextureIterator(self); })
        ...
        .def("get3DTextures", [](GpuShaderDescRcPtr & self) { return Texture3DIterator(self); });

    clsUniformData.def(py::init<>())
        ...
        .def(
            "getDouble",
            [](GpuShaderDesc::UniformData & self) -> double { return self.m_getDouble(); })
        .def("getBool", [](GpuShaderDesc::UniformData & self) -> bool { return self.m_getBool(); })
        .def(
            "getFloat3",
            [](GpuShaderDesc::UniformData & self) -> Float3 { return self.m_getFloat3(); })
        .def(
            "getVectorFloat",
            [](GpuShaderDesc::UniformData & self) -> py::array {
                return py::array(
                    py::dtype("float32"),
    ...
    clsTexture.def_readonly("textureName", &Texture::m_textureName)
    ...
    clsTextureIterator
        ...
        .def(
            "__getitem__",
            [](TextureIterator & it, int i) -> Texture {
                // GpuShaderDesc provides index check with exception
    ...
    clsTexture3D.def_readonly("textureName", &Texture3D::m_textureName)
    ...
    clsTexture3DIterator
        ...
        .def(
            "__getitem__",
            [](Texture3DIterator & it, int i) -> Texture3D {
                // GpuShaderDesc provides index check with exception
                const char * textureName = nullptr;
                const char * samplerName = nullptr;
                unsigned edgelen;
