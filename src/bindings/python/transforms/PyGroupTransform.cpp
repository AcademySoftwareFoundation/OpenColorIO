...
void bindPyGroupTransform(py::module & m)
{
    ...
    auto clsGroupTransform
        = py::class_<GroupTransform, GroupTransformRcPtr, Transform>(m.attr("GroupTransform"));

    auto clsTransformIterator
        = py::class_<TransformIterator>(clsGroupTransform, "TransformIterator");

    auto clsWriteFormatIterator
        = py::class_<WriteFormatIterator>(clsGroupTransform, "WriteFormatIterator");

    clsGroupTransform.def(py::init(&GroupTransform::Create), DOC(GroupTransform, Create))
        .def(
            py::init([](const std::vector<TransformRcPtr> transforms, TransformDirection dir) {
        ...
        .def("__len__", &GroupTransform::getNumTransforms, DOC(GroupTransform, getNumTransforms))
        .def(
            "__getitem__",
        ...
        .def(
            "appendTransform",
            &GroupTransform::appendTransform,
            "transform"_a,
            DOC(GroupTransform, appendTransform))
        .def(
            "prependTransform",
            &GroupTransform::prependTransform,
            "transform"_a,
            DOC(GroupTransform, prependTransform))
        .def(
            "write",
            [](GroupTransformRcPtr & self,
    ...
    clsWriteFormatIterator
        .def(
            "__len__",
            [](WriteFormatIterator & /* it */) { return GroupTransform::GetNumWriteFormats(); })
        .def(
            "__getitem__",
            [](WriteFormatIterator & it, int i) {
                it.checkIndex(i, GroupTransform::GetNumWriteFormats());
                return py::make_tuple(
                    GroupTransform::GetFormatNameByIndex(i),
