...
{
    auto clsProcessor = py::class_<Processor, ProcessorRcPtr>(m.attr("Processor"));

    auto clsTransformFormatMetadataIterator = py::class_<TransformFormatMetadataIterator>(
        clsProcessor,
        "TransformFormatMetadataIterator");

    clsProcessor.def("isNoOp", &Processor::isNoOp, DOC(Processor, isNoOp))
        .def(
            "hasChannelCrosstalk",
            &Processor::hasChannelCrosstalk,
            DOC(Processor, hasChannelCrosstalk))
        .def("getCacheID", &Processor::getCacheID, DOC(Processor, getCacheID))
        .def(
            "getProcessorMetadata",
            &Processor::getProcessorMetadata,
        ...
        .def(
            ...
            [](ProcessorRcPtr & self) { return TransformFormatMetadataIterator(self); })
        .def(
            "createGroupTransform",
            &Processor::createGroupTransform,
            DOC(Processor, createGroupTransform))
        .def(
            "getDynamicProperty",
            [](ProcessorRcPtr & self, DynamicPropertyType type) {
        ...
        .def("isDynamic", &Processor::isDynamic, DOC(Processor, isDynamic))
        .def(
    ...
    clsTransformFormatMetadataIterator
        .def(
            "__len__",
            [](TransformFormatMetadataIterator & it) { return it.m_obj->getNumTransforms(); })
        .def(
            "__getitem__",
        ...
        .def(
            "__next__",
            [](TransformFormatMetadataIterator & it) -> const FormatMetadata & {
