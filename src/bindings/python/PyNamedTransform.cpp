...
using NamedTransformCategoryIterator
    = PyIterator<NamedTransformRcPtr, IT_NAMED_TRANSFORM_DEFINITION_CATEGORY>;

using NamedTransformAliasIterator = PyIterator<NamedTransformRcPtr, IT_NAMED_TRANSFORM_ALIAS>;
...
void bindPyNamedTransform(py::module & m)
{
    ...
    auto clsNamedTransform
        = py::class_<NamedTransform, NamedTransformRcPtr>(m.attr("NamedTransform"));

    auto clsNamedTransformCategoryIterator = py::class_<NamedTransformCategoryIterator>(
        clsNamedTransform,
        "NamedTransformCategoryIterator");

    auto clsNamedTransformAliasIterator
        = py::class_<NamedTransformAliasIterator>(clsNamedTransform, "NamedTransformAliasIterator");
    ...
    clsNamedTransform
        ...
        .def(
            "__deepcopy__",
            [](const ConstNamedTransformRcPtr & self, py::dict) {
                return self->createEditableCopy();
            },
            "memo"_a)

        .def("getName", &NamedTransform::getName, DOC(NamedTransform, getName))
        .def(
            "setName",
            &NamedTransform::setName,
            "name"_a.none(false),
            DOC(NamedTransform, setName))

        // Aliases.
        .def("hasAlias", &NamedTransform::hasAlias, "alias"_a, DOC(NamedTransform, hasAlias))
        .def(
            "addAlias",
            &NamedTransform::addAlias,
            "alias"_a.none(false),
            DOC(NamedTransform, addAlias))
        .def(
            "removeAlias",
            &NamedTransform::removeAlias,
            "alias"_a.none(false),
            DOC(NamedTransform, removeAlias))
        .def(
            "getAliases",
            [](NamedTransformRcPtr & self) { return NamedTransformAliasIterator(self); })
        .def("clearAliases", &NamedTransform::clearAliases, DOC(NamedTransform, clearAliases))

        .def("getFamily", &NamedTransform::getFamily, DOC(NamedTransform, getFamily))
        .def(
            "setFamily",
            &NamedTransform::setFamily,
            "family"_a.none(false),
            DOC(NamedTransform, setFamily))
        .def("getDescription", &NamedTransform::getDescription, DOC(NamedTransform, getDescription))
        .def(
            "setDescription",
            &NamedTransform::setDescription,
            "description"_a.none(false),
            DOC(NamedTransform, setDescription))
        .def("getEncoding", &NamedTransform::getEncoding, DOC(NamedTransform, getEncoding))
        .def(
            "setEncoding",
            &NamedTransform::setEncoding,
            "encoding"_a.none(false),
            DOC(NamedTransform, setEncoding))

        // Transform
        .def(
            "getTransform",
            &NamedTransform::getTransform,
            "direction"_a,
            DOC(NamedTransform, getTransform))
        .def(
            "setTransform",
            &NamedTransform::setTransform,
            "transform"_a,
            "direction"_a,
        ...
        .def(
            "hasCategory",
            &NamedTransform::hasCategory,
            "category"_a,
            DOC(NamedTransform, hasCategory))
        .def(
            "addCategory",
            &NamedTransform::addCategory,
            "category"_a,
            DOC(NamedTransform, addCategory))
        .def(
            "removeCategory",
            &NamedTransform::removeCategory,
            "category"_a,
            DOC(NamedTransform, removeCategory))
        .def(
            "getCategories",
            [](NamedTransformRcPtr & self) { return NamedTransformCategoryIterator(self); })
        .def(
            "clearCategories",
            &NamedTransform::clearCategories,
    ...
    clsNamedTransformCategoryIterator
        .def(
            "__len__",
            [](NamedTransformCategoryIterator & it) { return it.m_obj->getNumCategories(); })
        .def(
            "__getitem__",
            [](NamedTransformCategoryIterator & it, int i) {
                it.checkIndex(i, it.m_obj->getNumCategories());
                return it.m_obj->getCategory(i);
            })
        .def(
            "__iter__",
            [](NamedTransformCategoryIterator & it) -> NamedTransformCategoryIterator & {
                return it;
            })
    ...
    clsNamedTransformAliasIterator
        .def("__len__", [](NamedTransformAliasIterator & it) { return it.m_obj->getNumAliases(); })
        .def(
            "__getitem__",
            [](NamedTransformAliasIterator & it, int i) {
                it.checkIndex(i, (int)it.m_obj->getNumAliases());
                return it.m_obj->getAlias(i);
            })
        .def(
            "__iter__",
            [](NamedTransformAliasIterator & it) -> NamedTransformAliasIterator & { return it; })
