...
void bindPyLegacyViewingPipeline(py::module & m)
{
    auto clsLegacyViewingPipeline
        = py::class_<LegacyViewingPipeline, LegacyViewingPipelineRcPtr>(
              ...
              .def(py::init(&LegacyViewingPipeline::Create), DOC(LegacyViewingPipeline, Create))
              .def(
                  "getDisplayViewTransform",
                  &LegacyViewingPipeline::getDisplayViewTransform,
                  DOC(LegacyViewingPipeline, getDisplayViewTransform))
              .def(
                  "setDisplayViewTransform",
                  &LegacyViewingPipeline::setDisplayViewTransform,
                  DOC(LegacyViewingPipeline, setDisplayViewTransform))
              .def(
                  "getLinearCC",
                  &LegacyViewingPipeline::getLinearCC,
                  DOC(LegacyViewingPipeline, getLinearCC))
              .def(
                  "setLinearCC",
                  &LegacyViewingPipeline::setLinearCC,
                  DOC(LegacyViewingPipeline, setLinearCC))
              .def(
                  "getColorTimingCC",
                  &LegacyViewingPipeline::getColorTimingCC,
                  DOC(LegacyViewingPipeline, getColorTimingCC))
              .def(
                  "setColorTimingCC",
                  &LegacyViewingPipeline::setColorTimingCC,
                  DOC(LegacyViewingPipeline, setColorTimingCC))
              .def(
                  "getChannelView",
                  &LegacyViewingPipeline::getChannelView,
                  DOC(LegacyViewingPipeline, getChannelView))
              .def(
                  "setChannelView",
                  &LegacyViewingPipeline::setChannelView,
                  DOC(LegacyViewingPipeline, setChannelView))
              .def(
                  "getDisplayCC",
                  &LegacyViewingPipeline::getDisplayCC,
                  DOC(LegacyViewingPipeline, getDisplayCC))
              .def(
                  "setDisplayCC",
                  &LegacyViewingPipeline::setDisplayCC,
                  DOC(LegacyViewingPipeline, setDisplayCC))
              .def(
                  "setLooksOverrideEnabled",
                  &LegacyViewingPipeline::setLooksOverrideEnabled,
                  DOC(LegacyViewingPipeline, setLooksOverrideEnabled))
              .def(
                  "getLooksOverrideEnabled",
                  &LegacyViewingPipeline::getLooksOverrideEnabled,
              ...
              .def(
                  "getLooksOverride",
                  &LegacyViewingPipeline::getLooksOverride,
                  DOC(LegacyViewingPipeline, getLooksOverride))
              .def(
                  "getProcessor",
                  [](LegacyViewingPipelineRcPtr & self,
                     const ConstConfigRcPtr & config,
                     const ConstContextRcPtr & context) {
                      ConstContextRcPtr usedContext
                          = context ? context : config->getCurrentContext();
