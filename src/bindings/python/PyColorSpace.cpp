...
void bindPyColorSpace(py::module & m)
{
    ...
    auto clsColorSpace = py::class_<ColorSpace, ColorSpaceRcPtr>(m.attr("ColorSpace"));

    auto clsColorSpaceCategoryIterator
        = py::class_<ColorSpaceCategoryIterator>(clsColorSpace, "ColorSpaceCategoryIterator");

    auto clsColorSpaceAliasIterator
        = py::class_<ColorSpaceAliasIterator>(clsColorSpace, "ColorSpaceAliasIterator");

    clsColorSpace.def(py::init([]() { return ColorSpace::Create(); }), DOC(ColorSpace, Create))
        .def(
            py::init([](ReferenceSpaceType referenceSpace) {
        ...
        .def(
            py::init([](ReferenceSpaceType referenceSpace,
                ...
                if (!encoding.empty())
                {
                    p->setEncoding(encoding.c_str());
                }
                if (!equalityGroup.empty())
                {
                    p->setEqualityGroup(equalityGroup.c_str());
                }
                ...
                if (!interopID.empty())
                {
                    p->setInteropID(interopID.c_str());
                }
        ...
        .def(
            "__deepcopy__",
            [](const ConstColorSpaceRcPtr & self, py::dict) { return self->createEditableCopy(); },
            "memo"_a)

        .def("getName", &ColorSpace::getName, DOC(ColorSpace, getName))
        .def("setName", &ColorSpace::setName, "name"_a.none(false), DOC(ColorSpace, setName))

        // Aliases.
        .def("hasAlias", &ColorSpace::hasAlias, "alias"_a, DOC(ColorSpace, hasAlias))
        .def("addAlias", &ColorSpace::addAlias, "alias"_a.none(false), DOC(ColorSpace, addAlias))
        .def(
            "removeAlias",
            &ColorSpace::removeAlias,
            "alias"_a.none(false),
            DOC(ColorSpace, removeAlias))
        .def("getAliases", [](ColorSpaceRcPtr & self) { return ColorSpaceAliasIterator(self); })
        .def("clearAliases", &ColorSpace::clearAliases, DOC(ColorSpace, clearAliases))

        .def("getFamily", &ColorSpace::getFamily, DOC(ColorSpace, getFamily))
        .def("setFamily", &ColorSpace::setFamily, "family"_a, DOC(ColorSpace, setFamily))
        .def("getEncoding", &ColorSpace::getEncoding, DOC(ColorSpace, getEncoding))
        .def("setEncoding", &ColorSpace::setEncoding, "encoding"_a, DOC(ColorSpace, setEncoding))
        .def("getEqualityGroup", &ColorSpace::getEqualityGroup, DOC(ColorSpace, getEqualityGroup))
        .def(
            "setEqualityGroup",
            &ColorSpace::setEqualityGroup,
            "equalityGroup"_a,
            DOC(ColorSpace, setEqualityGroup))
        .def("getDescription", &ColorSpace::getDescription, DOC(ColorSpace, getDescription))
        .def(
            "setDescription",
            &ColorSpace::setDescription,
            "description"_a,
            DOC(ColorSpace, setDescription))
        .def("getInteropID", &ColorSpace::getInteropID, DOC(ColorSpace, getInteropID))
        .def(
            "setInteropID",
            &ColorSpace::setInteropID,
            "interopID"_a,
            DOC(ColorSpace, setInteropID))
        .def(
            "getInterchangeAttribute",
            &ColorSpace::getInterchangeAttribute,
            "attrName"_a,
            DOC(ColorSpace, getInterchangeAttribute))
        .def(
            "setInterchangeAttribute",
            &ColorSpace::setInterchangeAttribute,
            "attrName"_a,
            "attrValue"_a,
            DOC(ColorSpace, setInterchangeAttribute))
        .def(
            "getInterchangeAttributes",
            &ColorSpace::getInterchangeAttributes,
            DOC(ColorSpace, getInterchangeAttributes))
        .def("getBitDepth", &ColorSpace::getBitDepth, DOC(ColorSpace, getBitDepth))
        .def("setBitDepth", &ColorSpace::setBitDepth, "bitDepth"_a, DOC(ColorSpace, setBitDepth))

        // Categories
        .def("hasCategory", &ColorSpace::hasCategory, "category"_a, DOC(ColorSpace, hasCategory))
        .def("addCategory", &ColorSpace::addCategory, "category"_a, DOC(ColorSpace, addCategory))
        .def(
            "removeCategory",
            &ColorSpace::removeCategory,
            "category"_a,
            DOC(ColorSpace, removeCategory))
        .def(
            "getCategories",
            [](ColorSpaceRcPtr & self) { return ColorSpaceCategoryIterator(self); })
        .def("clearCategories", &ColorSpace::clearCategories, DOC(ColorSpace, clearCategories))

        // Data
        .def("isData", &ColorSpace::isData, DOC(ColorSpace, isData))
        .def("setIsData", &ColorSpace::setIsData, "isData"_a, DOC(ColorSpace, setIsData))
        .def(
            "getReferenceSpaceType",
            &ColorSpace::getReferenceSpaceType,
            DOC(ColorSpace, getReferenceSpaceType))

        // Allocation
        .def("getAllocation", &ColorSpace::getAllocation, DOC(ColorSpace, getAllocation))
        .def(
            "setAllocation",
        ...
        .def(
            "getAllocationVars",
            [](ColorSpaceRcPtr & self) { return getAllocationVarsStdVec(self); })
        .def(
            "setAllocationVars",
            [](ColorSpaceRcPtr self, const std::vector<float> & vars) {
    ...
    clsColorSpaceCategoryIterator
        .def(
            "__len__",
            [](ColorSpaceCategoryIterator & it) { return it.m_obj->getNumCategories(); })
        .def(
            "__getitem__",
            [](ColorSpaceCategoryIterator & it, int i) {
                it.checkIndex(i, it.m_obj->getNumCategories());
                return it.m_obj->getCategory(i);
            })
        .def(
            "__iter__",
            [](ColorSpaceCategoryIterator & it) -> ColorSpaceCategoryIterator & { return it; })
    ...
    clsColorSpaceAliasIterator
        .def("__len__", [](ColorSpaceAliasIterator & it) { return it.m_obj->getNumAliases(); })
        .def(
            "__getitem__",
            [](ColorSpaceAliasIterator & it, int i) {
                it.checkIndex(i, (int)it.m_obj->getNumAliases());
                return it.m_obj->getAlias(i);
            })
        .def(
            "__iter__",
            [](ColorSpaceAliasIterator & it) -> ColorSpaceAliasIterator & { return it; })
