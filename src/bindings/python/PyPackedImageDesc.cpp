...
void bindPyPackedImageDesc(py::module & m)
{
    auto clsPackedImageDesc
        = py::class_<PyPackedImageDesc, PyImageDesc>(m.attr("PackedImageDesc"))

              .def(
                  py::init([](py::buffer & data, long width, long height, long numChannels) {
                      ...
                      p->m_img
                          = std::make_shared<PackedImageDesc>(info.ptr, width, height, numChannels);
              ...
              .def(
                  py::init([](py::buffer & data,
                      ...
                      p->m_img = std::make_shared<PackedImageDesc>(
                          info.ptr,
                          width,
                          height,
                          numChannels,
              ...
              .def(
                  py::init([](py::buffer & data,
                      ...
                      p->m_img
                          = std::make_shared<PackedImageDesc>(info.ptr, width, height, chanOrder);
              ...
              .def(
                  "getData",
                  [](const PyPackedImageDesc & self) {
                      PackedImageDescRcPtr p = self.getImg();
                      return py::array(
                          bitDepthToDtype(p->getBitDepth()),
              ...
              .def(
                  "getChannelOrder",
                  [](const PyPackedImageDesc & self) { return self.getImg()->getChannelOrder(); },
                  DOC(PackedImageDesc, getChannelOrder))
              .def(
                  "getNumChannels",
                  [](const PyPackedImageDesc & self) { return self.getImg()->getNumChannels(); },
