...
void bindPyBuiltinTransformRegistry(py::module & m)
{
    auto clsBuiltinTransformRegistry
        = py::class_<PyBuiltinTransformRegistry>(m.attr("BuiltinTransformRegistry"));

    auto clsBuiltinStyleIterator
        = py::class_<BuiltinStyleIterator>(clsBuiltinTransformRegistry, "BuiltinStyleIterator");

    auto clsBuiltinIterator
        = py::class_<BuiltinIterator>(clsBuiltinTransformRegistry, "BuiltinIterator");
    ...
    clsBuiltinTransformRegistry
        ...
        .def(
            "__iter__",
            [](PyBuiltinTransformRegistry & self) { return BuiltinStyleIterator(self); },
            DOC(BuiltinTransformRegistry, getBuiltinStyle))
        .def(
            "__len__",
            [](PyBuiltinTransformRegistry & self) { return self.getNumBuiltins(); },
            DOC(BuiltinTransformRegistry, getNumBuiltins))
        .def(
            "__getitem__",
            [](PyBuiltinTransformRegistry & self, const std::string & style) {
    ...
    clsBuiltinIterator
        ...
        .def(
            "__getitem__",
            [](BuiltinIterator & it, int i) {
                // BuiltinTransformRegistry provides index check with exception
                return py::make_tuple(
                    it.m_obj.getBuiltinStyle(i),
                    it.m_obj.getBuiltinDescription(i));
            })
        .def("__iter__", [](BuiltinIterator & it) -> BuiltinIterator & { return it; })
        .def("__next__", [](BuiltinIterator & it) {
            int i = it.nextIndex((int)it.m_obj.getNumBuiltins());
            return py::make_tuple(it.m_obj.getBuiltinStyle(i), it.m_obj.getBuiltinDescription(i));
        });
}
