...
template <typename InType, typename OutType>
void GenericScanlineHelper<InType, OutType>::init(
    const ImageDesc & srcImg,
    const ImageDesc & dstImg)
{
    m_yIndex = 0;
    ...
    m_useDstBuffer = (m_outOptimizedMode & PACKED_FLOAT_OPTIMIZATION) == PACKED_FLOAT_OPTIMIZATION;
...
void GenericScanlineHelper<InType, OutType>::prepRGBAScanline(float ** buffer, long & numPixels)
{
    ...
    *buffer = m_useDstBuffer ? (float *)(m_dstImg.m_rData + m_dstImg.m_yStrideBytes * m_yIndex)
    ...
    else
    {
        ...
        Generic<InType>::PackRGBAFromImageDesc(
            m_srcImg,
...
void GenericScanlineHelper<InType, OutType>::finishRGBAScanline()
{
    ...
    else
    {
        ...
        Generic<OutType>::UnpackRGBAToImageDesc(
            m_dstImg,
            &m_rgbaFloatBuffer[0],
