...
void Generic<Type>::PackRGBAFromImageDesc(
...
template <>
void Generic<float>::PackRGBAFromImageDesc(
    const GenericImageDesc & srcImg,
...
template <typename Type>
void Generic<Type>::UnpackRGBAToImageDesc(
    ...
    long imagePixelStartIndex)
{
    ...
    while (pixelsCopied < numPixelsToUnpack)
    {
        ...
        *bPtr = outBitDepthBuffer[4 * pixelsCopied + 2];
        if (aPtr)
            *aPtr = outBitDepthBuffer[4 * pixelsCopied + 3];
...
template <>
void Generic<float>::UnpackRGBAToImageDesc(
    GenericImageDesc & dstImg,
    float * inputBuffer,
