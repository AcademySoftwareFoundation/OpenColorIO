...
std::unique_ptr<GpuShaderClassWrapper> GpuShaderClassWrapper::CreateClassWrapper(
...
std::string MetalShaderClassWrapper::generateClassWrapperHeader(GpuShaderText & kw) const
{
    ...
    {
        throw Exception(
            ("Struct name must not start with a digit. Invalid className passed in: " + m_className)
                .c_str());
    }
    ...
    {
        kw.newLine() << separator << (param.m_isArray ? "constant " : "") << param.m_type << " "
                     << param.m_name;
        if (param.m_isArray)
        {
            kw.newLine() << ", int "
                         << GetArrayLengthVariableName(
    ...
    for (const auto & param : m_functionParameters)
    {
        ...
        else
        {
            ...
            kw.newLine() << "for(int i = 0; i < " << GetArrayLengthVariableName(variableName)
                         << "; ++i)";
            kw.newLine() << "{";
            kw.indent();
            ...
            kw.newLine() << "for(int i = " << GetArrayLengthVariableName(variableName) << "; i < "
                         << param.m_name.substr(
                                openAngledBracketPos + 1,
                                closeAngledBracketPos - openAngledBracketPos - 1)
                         << "; ++i)";
            kw.newLine() << "{";
            kw.indent();
...
std::string MetalShaderClassWrapper::generateClassWrapperFooter(
    GpuShaderText & kw,
    const std::string & ocioFunctionName) const
{
    ...
    for (const auto & param : m_functionParameters)
    {
        ...
        else
        {
            ...
            kw.newLine() << ", "
                         << GetArrayLengthVariableName(
                                param.m_name.substr(0, openAngledBracketPos));
        }
        separator = ", ";
    }
    kw.dedent();
...
std::string MetalShaderClassWrapper::getClassWrapperName(
...
{
    // We want the caller to always pass 3d luts first with their samplers, and then pass other
    // luts.
...
void MetalShaderClassWrapper::prepareClassWrapper(
    const std::string & resourcePrefix,
    const std::string & functionName,
...
{
    std::unique_ptr<MetalShaderClassWrapper> clonedWrapper
        = std::unique_ptr<MetalShaderClassWrapper>(new MetalShaderClassWrapper);
    *clonedWrapper = *this;
