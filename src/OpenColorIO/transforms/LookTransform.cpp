...
const char * LooksResultColorSpace(
    const Config & config,
...
const char * LookTransform::GetLooksResultColorSpace(
    const ConstConfigRcPtr & config,
    const ConstContextRcPtr & context,
    const char * looksStr)
{
    if (!looksStr || !*looksStr)
        return "";
...
std::ostream & operator<<(std::ostream & os, const LookTransform & t)
{
    ...
    os << ", dst=" << t.getDst();
    os << ", looks=" << t.getLooks();
    if (t.getSkipColorSpaceConversion())
        os << ", skipCSConversion";
    os << ">";
    return os;
}
    ...
    const LookParseResult::Tokens & lookTokens)
{
    ...
    for (unsigned int i = 0; i < lookTokens.size(); ++i)
    {
        ...
        if (!skipColorSpaceConversion && currentColorSpace.get() != processColorSpace.get())
        {
            // Default behavior is to bypass data color space.
            BuildColorSpaceOps(ops, config, context, currentColorSpace, processColorSpace, true);
            currentColorSpace = processColorSpace;
        }
    ...
    TransformDirection dir)
{
    ...
    BuildLookOps(ops, currentColorSpace, skipColorSpaceConversion, config, context, looks);
    ...
    const LookParseResult & looks)
{
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < options.size(); ++i)
        {
            ...
            try
            {
                RunLookTokens(tmpOps, cs, skipColorSpaceConversion, config, context, options[i]);
                success = true;
                break;
            }
