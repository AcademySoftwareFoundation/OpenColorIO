...
static const double gain
    = (highlight - shadow) / (1. - std::pow(10., multiFactor *(refBlack - refWhite)));
...
static const LogOpData::Params params{logSideSlope, logSideOffset, linSideSlope, linSideOffset};
...
{
    {
        auto RED_REDLOGFILM_RWG_to_ACES2065_1_Functor = [](OpRcPtrVec & ops) {
            ...
            MatrixOpData::MatrixArrayPtr rwg = build_conversion_matrix(
                RED_WIDE_GAMUT_RGB::primaries,
                ACES_AP0::primaries,
                ADAPTATION_BRADFORD);
