...
void GenerateOps(OpRcPtrVec & ops)
{
    ...
    auto GenerateLutValues = [](double in) -> float {
        ...
        {
            const double REF_PT = (7120. - 1520.) / 8000. * (100. / 55.) - std::log10(0.18);
...
void Generate_tonecurve_ops(OpRcPtrVec & ops)
{
    ...
    // Apply RRT shaper using the same quadratic B-spline as the CTL.
    {
        ...
        auto identity                   = GradingBSplineCurve::Create({
            {0.f, 0.f},
            {1.f, 1.f}
        });
    ...
    // Apply Cinema White/Black correction.
    {
        ...
        // Note: ACESlib.ODT_Common.ctl claims that using pow10(log10(0.02) for black improves
        // performance at 0, but that does not seem to be the case here, 0 input currently gives
        // about 4e-11 XYZ output either way.
...
void Generate_hdr_tonecurve_ops(OpRcPtrVec & ops, double Y_MAX)
{
    ...
    else if (Y_MAX == 2000.)
    {
        ...
        // clang-format on
        const float slopes[]
            = {0.f, 0.55982688f, 1.77532247f, 1.55f, 0.83637009f, 0.18505799f, 0.f};
        for (size_t i = 0; i < 7; ++i)
        {
            curve->setSlope(i, slopes[i]);
        }
    ...
    else if (Y_MAX == 4000.)
    {
        ...
        // clang-format on
        const float slopes[]
            = {0.f, 0.55982688f, 1.77532247f, 1.55f, 0.85652519f, 0.18474395f, 0.f};
        for (size_t i = 0; i < 7; ++i)
        {
            curve->setSlope(i, slopes[i]);
        }
    ...
    else if (Y_MAX == 108.)
    {
        ...
        // clang-format on
        const float slopes[]
            = {0.f, 0.55982688f, 1.77532247f, 1.55f, 0.68179646f, 0.17726487f, 0.f};
        for (size_t i = 0; i < 7; ++i)
        {
            curve->setSlope(i, slopes[i]);
        }
...
void Generate_sdr_primary_clamp_ops(OpRcPtrVec & ops, const Primaries & limitPrimaries)
{
    ...
    MatrixOpData::MatrixArrayPtr matrix2 = rgb2xyz_from_xy(limitPrimaries);
    ...
    bool scale_white)
{
    ...
    const float upperBound
        = 8.f * (128.f + 768.f * (std::log(peak_luminance / 100.f) / log(10000.f / 100.f)));
    CreateRangeOp(ops, 0., upperBound, 0., upperBound, TRANSFORM_DIR_FORWARD);

    CreateMatrixOp(ops, matrixToAP1, TRANSFORM_DIR_INVERSE);

    // Display rendering
    CreateFixedFunctionOp(
        ops,
        FixedFunctionOpData::ACES_OUTPUT_TRANSFORM_20_FWD,
        {
            peak_luminance,
            limiting_pri.m_red.m_xy[0],
            limiting_pri.m_red.m_xy[1],
            limiting_pri.m_grn.m_xy[0],
            limiting_pri.m_grn.m_xy[1],
            limiting_pri.m_blu.m_xy[0],
            limiting_pri.m_blu.m_xy[1],
            limiting_pri.m_wht.m_xy[0],
            limiting_pri.m_wht.m_xy[1],
        });

    // Post transform clamp
    const double normPeakLuminance = peak_luminance / 100.f;
    CreateRangeOp(ops, 0., normPeakLuminance, 0., normPeakLuminance, TRANSFORM_DIR_FORWARD);
...
{
    {
        auto ACES_AP0_to_CIE_XYZ_D65_BFD_Functor = [](OpRcPtrVec & ops) {
            ...
            // A neutral value of [1.,1.,1] in AP0 maps to the XYZ value of D65
            // ([0.9504..., 1., 1.089...]).
            MatrixOpData::MatrixArrayPtr matrix
    ...
    {
        auto ACES_AP1_to_CIE_XYZ_D65_BFD_Functor
            = [](OpRcPtrVec & ops) { AP1_to_CIE_XYZ_D65::GenerateOps(ops); };
    ...
    }
    {
        auto ACEScc_to_ACES2065_1_Functor = [](OpRcPtrVec & ops) {
            auto GenerateLutValues = [](double input) -> float {
                ...
                // The CTL clamps at HALF_MAX, but it's better to avoid a slope discontinuity in a
                // LUT.
            ...
            CreateRangeOp(ops, -0.36, 1.5, 0.00, 1.0, TRANSFORM_DIR_FORWARD);
    ...
    }
    {
        auto GAMUT_COMP_13_Functor = [](OpRcPtrVec & ops) {
            ...
            CreateFixedFunctionOp(
                ops,
                FixedFunctionOpData::ACES_GAMUT_COMP_13_FWD,
        ...
        registry.addBuiltin(
            ...
            "LMT (applied in ACES2065-1) to compress scene-referred values from common cameras "
            "into the AP1 gamut",
...

    {
        auto ACES2065_1_to_CIE_XYZ_cinema_d60sim_1_1_Functor = [](OpRcPtrVec & ops) {
            ...
            MatrixOpData::MatrixArrayPtr matrix = rgb2xyz_from_xy(ACES_AP1::primaries);
            CreateMatrixOp(ops, matrix, TRANSFORM_DIR_FORWARD);
        };
...

    {
        auto ACES2065_1_to_CIE_XYZ_cinema_d60sim_dci_1_0_Functor = [](OpRcPtrVec & ops) {
            ...
            MatrixOpData::MatrixArrayPtr matrix2 = build_vonkries_adapt(
                WHITEPOINT::DCI_XYZ,
                WHITEPOINT::D65_XYZ,
                ADAPTATION_BRADFORD);
            CreateMatrixOp(ops, matrix2, TRANSFORM_DIR_FORWARD);
    ...
    {
        auto ACES2065_1_to_CIE_XYZ_hdr_video_1000nits_rec2020lim_1_1_Functor
            = [](OpRcPtrVec & ops) {
                  ACES_OUTPUT::Generate_RRT_preamble_ops(ops);
    ...
    {
        auto ACES2065_1_to_CIE_XYZ_hdr_video_2000nits_rec2020lim_1_1_Functor
            = [](OpRcPtrVec & ops) {
                  ACES_OUTPUT::Generate_RRT_preamble_ops(ops);
    ...
    {
        auto ACES2065_1_to_CIE_XYZ_hdr_video_4000nits_rec2020lim_1_1_Functor
            = [](OpRcPtrVec & ops) {
                  ACES_OUTPUT::Generate_RRT_preamble_ops(ops);
    ...
    const std::vector<ACES2OutputTransform> aces2_output_transforms{
        ...
        {                     "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - SDR-100nit-REC709_2.0",
         "Component of ACES 2 Output Transforms for 100 nit SDR Rec709",  100.f,
         REC709::primaries,
         REC709::primaries,
         1.f, false  },
        {                     "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - SDR-100nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 100 nit SDR P3-D65",  100.f,
         P3_D65::primaries,
         P3_D65::primaries,
         1.f, false  },
        {                     "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-108nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 108 nit HDR P3-D65",  225.f, // = 108 * (100/48);
 P3_D65::primaries,
         P3_D65::primaries,
         0.48f, false},
        {                     "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-300nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 300 nit HDR P3-D65",  625.f, // = 300 * (100/48);
 P3_D65::primaries,
         P3_D65::primaries,
         0.48f, false},
        {                     "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-500nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 500 nit HDR P3-D65",  500.f,
         P3_D65::primaries,
         P3_D65::primaries,
         1.f, false  },
        {                    "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-1000nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 1000 nit HDR P3-D65", 1000.f,
         P3_D65::primaries,
         P3_D65::primaries,
         1.f, false  },
        {                    "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-2000nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 2000 nit HDR P3-D65", 2000.f,
         P3_D65::primaries,
         P3_D65::primaries,
         1.f, false  },
        {                    "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-4000nit-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 4000 nit HDR P3-D65", 4000.f,
         P3_D65::primaries,
         P3_D65::primaries,
         1.f, false  },
        {                    "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-500nit-REC2020_2.0",
         "Component of ACES 2 Output Transforms for 500 nit HDR Rec2020",  500.f,
         REC2020::primaries,
         REC2020::primaries,
         1.f, false  },
        {                   "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-1000nit-REC2020_2.0",
         "Component of ACES 2 Output Transforms for 1000 nit HDR Rec2020", 1000.f,
         REC2020::primaries,
         REC2020::primaries,
         1.f, false  },
        {                   "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-2000nit-REC2020_2.0",
         "Component of ACES 2 Output Transforms for 2000 nit HDR Rec2020", 2000.f,
         REC2020::primaries,
         REC2020::primaries,
         1.f, false  },
        {                   "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-4000nit-REC2020_2.0",
         "Component of ACES 2 Output Transforms for 4000 nit HDR Rec2020", 4000.f,
         REC2020::primaries,
         REC2020::primaries,
         1.f, false  },
        //
        ...
        {   "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - SDR-100nit-REC709-D60-in-REC709-D65_2.0",
         "Component of ACES 2 Output Transforms for 100 nit SDR Rec709 simulating D60 white in "
         "Rec709",  100.f,
        ...
        {           "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - SDR-100nit-P3-D60-in-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 100 nit SDR P3-D60 simulating D60 white in "
         "P3-D65",  100.f,
         P3_D60::primaries,
         P3_D65::primaries,
         1.f,  true  },
        ...
        {           "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-108nit-P3-D60-in-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 108 nit HDR P3-D60 simulating D60 white in "
         "P3-D65",  225.f, // = 108 * (100/48);
 P3_D60::primaries,
         P3_D65::primaries,
         0.48f,  true},
        {            "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-300nit-P3-D60-in-XYZ-E_2.0",
         "Component of ACES 2 Output Transforms for 300 nit HDR P3-D60 simulating D60 white in "
         "XYZ-E",  625.f, // = 300 * (100/48);
 P3_D60::primaries,
         CIE_XYZ_ILLUM_E::primaries,
         0.48f,  true},
        {           "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-500nit-P3-D60-in-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 500 nit HDR P3-D60 simulating D60 white in "
         "P3-D65",  500.f,
         P3_D60::primaries,
         P3_D65::primaries,
         1.f,  true  },
        {          "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-1000nit-P3-D60-in-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 1000 nit HDR P3-D60 simulating D60 white in "
         "P3-D65", 1000.f,
         P3_D60::primaries,
         P3_D65::primaries,
         1.f,  true  },
        {          "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-2000nit-P3-D60-in-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 2000 nit HDR P3-D60 simulating D60 white in "
         "P3-D65", 2000.f,
         P3_D60::primaries,
         P3_D65::primaries,
         1.f,  true  },
        {          "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-4000nit-P3-D60-in-P3-D65_2.0",
         "Component of ACES 2 Output Transforms for 4000 nit HDR P3-D60 simulating D60 white in "
         "P3-D65", 4000.f,
         P3_D60::primaries,
         P3_D65::primaries,
         1.f,  true  },
        ...
        { "ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-500nit-REC2020-D60-in-REC2020-D65_2.0",
         "Component of ACES 2 Output Transforms for 500 nit HDR Rec2020 simulating D60 white in "
         "Rec2020",  500.f,
         REC2020_D60::primaries,
         REC2020::primaries,
         1.f,  true  },
        {"ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-1000nit-REC2020-D60-in-REC2020-D65_2.0",
         "Component of ACES 2 Output Transforms for 1000 nit HDR Rec2020 simulating D60 white in "
         "Rec2020", 1000.f,
         REC2020_D60::primaries,
         REC2020::primaries,
         1.f,  true  },
        {"ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-2000nit-REC2020-D60-in-REC2020-D65_2.0",
         "Component of ACES 2 Output Transforms for 2000 nit HDR Rec2020 simulating D60 white in "
         "Rec2020", 2000.f,
         REC2020_D60::primaries,
         REC2020::primaries,
         1.f,  true  },
        {"ACES-OUTPUT - ACES2065-1_to_CIE-XYZ-D65 - HDR-4000nit-REC2020-D60-in-REC2020-D65_2.0",
         "Component of ACES 2 Output Transforms for 4000 nit HDR Rec2020 simulating D60 white in "
         "Rec2020", 4000.f,
         REC2020_D60::primaries,
         REC2020::primaries,
         1.f,  true  }
