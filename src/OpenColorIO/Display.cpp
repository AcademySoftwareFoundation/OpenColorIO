...
{
    return std::find_if(displays.begin(), displays.end(), [name](DisplayPair & display) {
...
{
    return std::find_if(displays.begin(), displays.end(), [name](const DisplayPair & display) {
        return 0 == Platform::Strcasecmp(name.c_str(), display.first.c_str());
    });
}
...
{
    return std::find_if(vec.begin(), vec.end(), [name](const View & view) {
...
{
    return std::find_if(vec.begin(), vec.end(), [name](View & view) {
        return 0 == Platform::Strcasecmp(name.c_str(), view.m_name.c_str());
    });
}
    ...
    const char * description)
{
    if (displayColorSpace
        && 0 == Platform::Strcasecmp(displayColorSpace, OCIO_VIEW_USE_DISPLAY_NAME))
    {
...
void ComputeDisplays(
    StringUtils::StringVec & displayCache,
