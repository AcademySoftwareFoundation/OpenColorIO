    ...
    std::vector<float> & res)
{
    ...
    const size_t size = w * h * d * (type == GpuShaderDesc::TEXTURE_RGB_CHANNEL ? 3 : 1);
    res.resize(size);
...
public:
    struct Texture
    {
            ...
            , m_textureShaderBindingIndex(textureShaderBindingIndex)
        {
            ...
            {
                std::stringstream ss;
                ss << "The texture buffer size is invalid: [" << w << " x " << h << " x " << d
                   << "].";
    ...
    struct Uniform
    {
        Uniform(
            const char * name,
            const GpuShaderCreator::DoubleGetter & getDouble,
        ...
        Uniform(
            const char * name,
            const GpuShaderCreator::Float3Getter & getFloat3,
            std::size_t bufferOffset)
            : Uniform(name)
        {
...
public:
        ...
        const float * values)
    {
        ...
        {
            std::stringstream ss;
            ss << "1D LUT size exceeds the maximum: " << width << " > " << get1dLutMaxWidth();
            throw Exception(ss.str().c_str());
        }
        unsigned textureShaderBindingIndex
            = static_cast<unsigned>(m_textures.size()) + static_cast<unsigned>(m_textures3D.size());
        ...
        Interpolation & interpolation) const
    {
        if (index >= m_textures.size())
        {
            ...
            ss << "1D LUT access error: index = " << index << " where size = " << m_textures.size();
            throw Exception(ss.str().c_str());
        }
        ...
        {
            std::stringstream ss;
            ss << "1D LUT cannot have more than two dimensions: " << t.m_dimensions << " > 2";
            throw Exception(ss.str().c_str());
        }
        ...
        const float * values)
    {
        ...
        {
            std::stringstream ss;
            ss << "3D LUT edge length exceeds the maximum: " << edgelen << " > "
               << get3dLutMaxLength();
            throw Exception(ss.str().c_str());
        }
    ...
    unsigned getNumUniforms() const { return (unsigned)m_uniforms.size(); }
    ...
    bool addUniform(const char * name, const GpuShaderCreator::BoolGetter & getter)
    {
        ...
        m_uniforms.emplace_back(name, getter, m_uniformBufferSize);
        m_uniformBufferSize
            += GPU_INT_SIZE; // bool not supported for buffered uniforms, using int instead
        return true;
    }
    ...
    std::size_t getUniformBufferSize() const { return m_uniformBufferSize; }
    Textures m_textures;
...
const char * GenericGpuShaderDesc::getUniform(unsigned index, GpuShaderDesc::UniformData & data)
    const
{
...
bool GenericGpuShaderDesc::addUniform(
    const char * name,
...
unsigned GenericGpuShaderDesc::addTexture(
    const char * textureName,
    const char * samplerName,
    unsigned width,
    unsigned height,
    ...
    const float * values)
{
    return getImplGeneric()->addTexture(
               textureName,
               samplerName,
               width,
               height,
...
unsigned GenericGpuShaderDesc::add3DTexture(
    const char * textureName,
    const char * samplerName,
    unsigned edgelen,
    Interpolation interpolation,
