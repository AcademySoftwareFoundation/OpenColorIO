...
LoggingLevel LoggingLevelFromString(const char * s)
{
    ...
    else if (str == "1" || str == "warning")
        return LOGGING_LEVEL_WARNING;
    else if (str == "2" || str == "info")
        return LOGGING_LEVEL_INFO;
    else if (str == "3" || str == "debug")
...
const char * BitDepthToString(BitDepth bitDepth)
{
    if (bitDepth == BIT_DEPTH_UINT8)
        ...
        return "12ui";
    else if (bitDepth == BIT_DEPTH_UINT14)
        return "14ui";
    else if (bitDepth == BIT_DEPTH_UINT16)
        return "16ui";
    else if (bitDepth == BIT_DEPTH_UINT32)
        return "32ui";
...
{
    if (alloc == ALLOCATION_UNIFORM)
        return "uniform";
    else if (alloc == ALLOCATION_LG2)
        return "lg2";
    return "unknown";
}
...
Allocation AllocationFromString(const char * s)
{
    ...
    if (str == "uniform")
        return ALLOCATION_UNIFORM;
    else if (str == "lg2")
        return ALLOCATION_LG2;
...
const char * InterpolationToString(Interpolation interp)
{
    if (interp == INTERP_NEAREST)
        ...
        return "linear";
    else if (interp == INTERP_TETRAHEDRAL)
        return "tetrahedral";
    else if (interp == INTERP_BEST)
        return "best";
    else if (interp == INTERP_DEFAULT)
        return "default";
...
GpuLanguage GpuLanguageFromString(const char * s)
{
    ...
    if (str == "cg")
        return GPU_LANGUAGE_CG;
    else if (str == "glsl_1.2")
        return GPU_LANGUAGE_GLSL_1_2;
    else if (str == "glsl_1.3")
        return GPU_LANGUAGE_GLSL_1_3;
    else if (str == "glsl_4.0")
        return GPU_LANGUAGE_GLSL_4_0;
    else if (str == "glsl_vk_4.6")
        return GPU_LANGUAGE_GLSL_VK_4_6;
    else if (str == "glsl_es_1.0")
        return GPU_LANGUAGE_GLSL_ES_1_0;
    else if (str == "glsl_es_3.0")
        return GPU_LANGUAGE_GLSL_ES_3_0;
    else if (str == "hlsl_sm_5.0")
        return GPU_LANGUAGE_HLSL_SM_5_0;
    else if (str == "osl_1")
        return LANGUAGE_OSL_1;
    else if (str == "msl_2")
        return GPU_LANGUAGE_MSL_2_0;
...
{
    if (mode == ENV_ENVIRONMENT_LOAD_PREDEFINED)
        return "loadpredefined";
    else if (mode == ENV_ENVIRONMENT_LOAD_ALL)
        return "loadall";
    return "unknown";
}
...
EnvironmentMode EnvironmentModeFromString(const char * s)
{
    ...
    if (str == "loadpredefined")
        return ENV_ENVIRONMENT_LOAD_PREDEFINED;
    else if (str == "loadall")
        return ENV_ENVIRONMENT_LOAD_ALL;
...
{
    if (style == RANGE_NO_CLAMP)
        return "noClamp";
    else if (style == RANGE_CLAMP)
        return "Clamp";
    return "Clamp";
}
...
const char * FixedFunctionStyleToString(FixedFunctionStyle style)
{
    switch (style)
    {
        ...
        case FIXED_FUNCTION_LIN_TO_GAMMA_LOG:
            return "Lin_TO_GammaLog";
        case FIXED_FUNCTION_LIN_TO_DOUBLE_LOG:
            return "Lin_TO_DoubleLog";
...
const char * GradingStyleToString(GradingStyle style)
{
    ...
    {
        case GRADING_LIN:
            return GRADING_STYLE_LINEAR;
        case GRADING_LOG:
            return GRADING_STYLE_LOGARITHMIC;
        case GRADING_VIDEO:
            return GRADING_STYLE_VIDEO;
    }
...
GradingStyle GradingStyleFromString(const char * style)
{
    ...
    if (str == GRADING_STYLE_LINEAR)
        return GRADING_LIN;
    else if (str == GRADING_STYLE_LOGARITHMIC)
        return GRADING_LOG;
    else if (str == GRADING_STYLE_VIDEO)
        return GRADING_VIDEO;
...
ExposureContrastStyle ExposureContrastStyleFromString(const char * style)
{
    ...
    if (str == EC_STYLE_LINEAR)
        return EXPOSURE_CONTRAST_LINEAR;
    else if (str == EC_STYLE_VIDEO)
        return EXPOSURE_CONTRAST_VIDEO;
    else if (str == EC_STYLE_LOGARITHMIC)
...
const char * NegativeStyleToString(NegativeStyle style)
{
    ...
    {
        case NEGATIVE_CLAMP:
            return NEGATIVE_STYLE_CLAMP;
        case NEGATIVE_MIRROR:
            return NEGATIVE_STYLE_MIRROR;
        case NEGATIVE_PASS_THRU:
            return NEGATIVE_STYLE_PASS_THRU;
        case NEGATIVE_LINEAR:
            return NEGATIVE_STYLE_LINEAR;
    }
...
NegativeStyle NegativeStyleFromString(const char * style)
{
    ...
    if (str == NEGATIVE_STYLE_MIRROR)
        return NEGATIVE_MIRROR;
    else if (str == NEGATIVE_STYLE_PASS_THRU)
        return NEGATIVE_PASS_THRU;
    else if (str == NEGATIVE_STYLE_CLAMP)
        return NEGATIVE_CLAMP;
    else if (str == NEGATIVE_STYLE_LINEAR)
...
bool StringToInt(int * ival, const char * str, bool failIfLeftoverChars)
{
    ...
    char c = 0;
    if (!(i >> *ival) || (failIfLeftoverChars && i.get(c)))
        return false;
    return true;
}
...
StringUtils::StringVec SplitStringEnvStyle(const std::string & str)
{
    ...
    if (foundComma != std::string::npos || foundColon != std::string::npos)
    {
        int currentPos = 0;
        while (s.size() > 0 && currentPos <= (int)s.size())
        {
            int nameEndPos
                = FindEndOfName(s, currentPos, foundComma != std::string::npos ? ',' : ':');
...
std::string JoinStringEnvStyle(const StringUtils::StringVec & outputvec)
{
    ...
    if (firstValue.find_first_of(",:") != std::string::npos && firstValue.size() > 1
        && firstValue[0] != '"' && firstValue[firstValue.size() - 1] != '"')
    {
        result += "\"" + firstValue + "\"";
    }
    else
    ...
    {
        if (outputvec[i].find_first_of(",:") != std::string::npos && outputvec[i].size() > 1
            && outputvec[i][0] != '"' && outputvec[i][outputvec[i].size() - 1] != '"')
        {
            result += ", \"" + outputvec[i] + "\"";
        }
        else
...
StringUtils::StringVec IntersectStringVecsCaseIgnore(
    const StringUtils::StringVec & vec1,
...
int FindInStringVecCaseIgnore(const StringUtils::StringVec & vec, const std::string & str)
{
    ...
    {
        if (StringUtils::Lower(vec[i]) == teststr)
