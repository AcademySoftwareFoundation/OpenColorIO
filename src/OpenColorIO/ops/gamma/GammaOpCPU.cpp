...
{
protected:
    explicit GammaMoncurveOpCPU(ConstGammaOpDataRcPtr &)
        : OpCPU()
    {
    }
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaMoncurveOpCPUFwdSSE>(gamma);
            else
#endif
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaMoncurveOpCPURevSSE>(gamma);
            else
#endif
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaMoncurveMirrorOpCPUFwdSSE>(gamma);
            else
#endif
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaMoncurveMirrorOpCPURevSSE>(gamma);
            else
#endif
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaBasicOpCPUSSE>(gamma);
            else
#endif
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaBasicMirrorOpCPUSSE>(gamma);
            else
#endif
...
#if OCIO_USE_SSE2
            if (fastPower)
                return std::make_shared<GammaBasicPassThruOpCPUSSE>(gamma);
            else
#endif
...
void GammaBasicMirrorOpCPU::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    {
        const float sign[4]
            = {std::copysign(1.0f, in[0]),
               std::copysign(1.0f, in[1]),
               std::copysign(1.0f, in[2]),
               std::copysign(1.0f, in[3])};
        const float pixel[4]
            = {std::fabs(in[0]), std::fabs(in[1]), std::fabs(in[2]), std::fabs(in[3])};
...
void GammaBasicPassThruOpCPUSSE::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        data = _mm_or_ps(_mm_and_ps(flag, data), _mm_andnot_ps(flag, pixel));
...
void GammaMoncurveOpCPUFwdSSE::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    const __m128 scale = _mm_set_ps(m_alpha.scale, m_blue.scale, m_green.scale, m_red.scale);

    const __m128 offset = _mm_set_ps(m_alpha.offset, m_blue.offset, m_green.offset, m_red.offset);

    const __m128 gamma = _mm_set_ps(m_alpha.gamma, m_blue.gamma, m_green.gamma, m_red.gamma);

    const __m128 breakPnt
        = _mm_set_ps(m_alpha.breakPnt, m_blue.breakPnt, m_green.breakPnt, m_red.breakPnt);

    const __m128 slope = _mm_set_ps(m_alpha.slope, m_blue.slope, m_green.slope, m_red.slope);
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        data = _mm_or_ps(_mm_and_ps(flag, data), _mm_andnot_ps(flag, _mm_mul_ps(pixel, slope)));
...
void GammaMoncurveOpCPUFwd::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    const float grn[5]
        = {m_green.scale, m_green.offset, m_green.gamma, m_green.breakPnt, m_green.slope};
    const float blu[5] = {m_blue.scale, m_blue.offset, m_blue.gamma, m_blue.breakPnt, m_blue.slope};
    const float alp[5]
        = {m_alpha.scale, m_alpha.offset, m_alpha.gamma, m_alpha.breakPnt, m_alpha.slope};
...
void GammaMoncurveOpCPURev::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    const float red[5] = {m_red.gamma, m_red.scale, m_red.offset, m_red.breakPnt, m_red.slope};
    const float grn[5]
        = {m_green.gamma, m_green.scale, m_green.offset, m_green.breakPnt, m_green.slope};
    const float blu[5] = {m_blue.gamma, m_blue.scale, m_blue.offset, m_blue.breakPnt, m_blue.slope};
    const float alp[5]
        = {m_alpha.gamma, m_alpha.scale, m_alpha.offset, m_alpha.breakPnt, m_alpha.slope};
