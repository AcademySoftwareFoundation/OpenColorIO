...
bool operator==(const GradingRGBMSW & lhs, const GradingRGBMSW & rhs)
{
    return lhs.m_red == rhs.m_red && lhs.m_green == rhs.m_green && lhs.m_blue == rhs.m_blue
           && lhs.m_master == rhs.m_master && lhs.m_start == rhs.m_start
...
{
    return lhs.m_blacks == rhs.m_blacks && lhs.m_whites == rhs.m_whites
           && lhs.m_highlights == rhs.m_highlights && lhs.m_midtones == rhs.m_midtones
           && lhs.m_shadows == rhs.m_shadows && lhs.m_scontrast == rhs.m_scontrast;
}
...
void GradingTone::validate() const
{
...

    {
        ...
        if (bd.m_red < MinBMWTol || bd.m_green < MinBMWTol || bd.m_blue < MinBMWTol
        ...
        if (bd.m_red > MaxBMWTol || bd.m_green > MaxBMWTol || bd.m_blue > MaxBMWTol
    ...
    }
    {
        ...
        if (midtones.m_red < MinBMWTol || midtones.m_green < MinBMWTol
            || midtones.m_blue < MinBMWTol || midtones.m_master < MinBMWTol)
        {
            std::ostringstream oss;
            oss << "GradingTone midtones '" << midtones << "' are below lower bound (" << MinBMW
                << ").";
            throw Exception(oss.str().c_str());
        }
        ...
        if (midtones.m_red > MaxBMWTol || midtones.m_green > MaxBMWTol
            || midtones.m_blue > MaxBMWTol || midtones.m_master > MaxBMWTol)
        {
            std::ostringstream oss;
            oss << "GradingTone midtones '" << midtones << "' are above upper bound (" << MaxBMW
                << ").";
            throw Exception(oss.str().c_str());
        }
    }
    {
        ...
        if (dw.m_red < MinBMWTol || dw.m_green < MinBMWTol || dw.m_blue < MinBMWTol
            || dw.m_master < MinBMWTol)
        {
            std::ostringstream oss;
            oss << "GradingTone whites '" << dw << "' are below lower bound (" << MinBMW << ").";
            throw Exception(oss.str().c_str());
        }
        ...
        if (dw.m_red > MaxBMWTol || dw.m_green > MaxBMWTol || dw.m_blue > MaxBMWTol
            || dw.m_master > MaxBMWTol)
        {
            std::ostringstream oss;
            oss << "GradingTone white '" << dw << "' are above upper bound (" << MaxBMW << ").";
            throw Exception(oss.str().c_str());
        }
    }
    {
        ...
        if (shadows.m_red < MinSHTol || shadows.m_green < MinSHTol || shadows.m_blue < MinSHTol
            || shadows.m_master < MinSHTol)
        {
            std::ostringstream oss;
            oss << "GradingTone shadows '" << shadows << "' are below lower bound (" << MinSH
                << ").";
            throw Exception(oss.str().c_str());
        }
        ...
        if (shadows.m_red > MaxSHTol || shadows.m_green > MaxSHTol || shadows.m_blue > MaxSHTol
            || shadows.m_master > MaxSHTol)
        {
            std::ostringstream oss;
            oss << "GradingTone shadows '" << shadows << "' are above upper bound (" << MaxSH
                << ").";
            throw Exception(oss.str().c_str());
        }
    }
    {
        ...
        if (hl.m_red < MinSHTol || hl.m_green < MinSHTol || hl.m_blue < MinSHTol
            || hl.m_master < MinSHTol)
        {
            std::ostringstream oss;
            oss << "GradingTone highlights '" << hl << "' are below lower bound (" << MinSH << ").";
            throw Exception(oss.str().c_str());
        }
        // Check that pivot is not overlapping start.
        ...
        if (hl.m_red > MaxSHTol || hl.m_green > MaxSHTol || hl.m_blue > MaxSHTol
            || hl.m_master > MaxSHTol)
        {
            std::ostringstream oss;
            oss << "GradingTone highlights '" << hl << "' are above upper bound (" << MaxSH << ").";
            throw Exception(oss.str().c_str());
        }
    }
    {
        ...
        {
            std::ostringstream oss;
            oss << "GradingTone s-contrast '" << m_scontrast << "' is below lower bound (" << MinWSC
                << ").";
            throw Exception(oss.str().c_str());
        }
        ...
        {
            std::ostringstream oss;
            oss << "GradingTone s-contrast '" << m_scontrast << "' is above upper bound (" << MaxSC
                << ").";
            throw Exception(oss.str().c_str());
        }
    }
...
{
    return IsIdentity(value.m_blacks) && IsIdentity(value.m_shadows) && IsIdentity(value.m_midtones)
           && IsIdentity(value.m_highlights) && IsIdentity(value.m_whites)
           && value.m_scontrast == 1.;
}
...
void GradingTonePreRender::FromStyle(
    GradingStyle style,
    float & top,
    float & topSC,
    float & bottom,
    ...
    double x1)
{
    ...
    const double fR
        = y1 * (1. - tR) * (1. - tR) + y2 * (2. - tR) * tR + m2 * (tR - 1.) * tR * (x2 - x1);
...
void GradingTonePreRender::update(const GradingTone & v)
{
...

    {
        ...
        const double new_start
            = HighlightFwdEval(startw, m_highlightsStart, m_highlightsWidth, master);
        const double new_end
            = HighlightFwdEval(startw + widthw, m_highlightsStart, m_highlightsWidth, master);
    ...
    }
    {
        ...
        const double new_end
            = ShadowFwdEval(startb - widthb, m_shadowsWidth, m_shadowsStart, master);
...
void GradingTonePreRender::mids_precompute(const GradingTone & v, float top, float bottom)
{
    ...
    for (const auto channel : {R, G, B, M})
    {
        ...
        if (mid_adj != 1.f)
        {
            ...
            const float center
                = Clamp(static_cast<float>(v.m_midtones.m_start), min_cent, max_cent);
...
void GradingTonePreRender::highlightShadow_precompute(const GradingTone & v)
{
    for (const auto isShadow : {false, true})
    {
        for (const auto channel : {R, G, B, M})
        {
            ...
            {
                const float start
                    = static_cast<float>(isShadow ? m_shadowsStart : m_highlightsStart);
                const float pivot
                    = static_cast<float>(isShadow ? m_shadowsWidth : m_highlightsWidth);
...
void GradingTonePreRender::whiteBlack_precompute(const GradingTone & v)
{
    for (const auto isBlack : {false, true})
    {
        for (const auto channel : {R, G, B, M})
        {
            ...
            const float start = static_cast<float>(isBlack ? m_blacksStart : m_whitesStart);
            const float width = static_cast<float>(isBlack ? m_blacksWidth : m_whitesWidth);
...
void GradingTonePreRender::scontrast_precompute(
    const GradingTone & v,
    float topSC,
    float bottom,
    float pivot)
