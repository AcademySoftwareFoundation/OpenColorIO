...
public:
    ...
    public:
        ...
        template <typename T> void setRGB(const T * v3);

        template <typename T> void setRGBA(const T * v4);

        inline const double & operator[](unsigned long index) const { return m_values[index]; }

        inline double & operator[](unsigned long index) { return m_values[index]; }
    ...
    public:
        ...
        template <typename T> void setRGB(const T * values);
...
public:
    ...
    template <typename T> void setRGBA(const T * values);

    inline const Offsets & getOffsets() const { return m_offsets; }

    inline Offsets & getOffsets() { return m_offsets; }

    double getOffsetValue(unsigned long index) const;

    inline void setRGBOffsets(const float * offsets) { m_offsets.setRGB(offsets); }

    inline void setRGBAOffsets(const float * offsets) { m_offsets.setRGBA(offsets); }

    inline void setRGBAOffsets(const double * offsets) { m_offsets.setRGBA(offsets); }

    inline void setOffsets(const Offsets & offsets) { m_offsets = offsets; }
