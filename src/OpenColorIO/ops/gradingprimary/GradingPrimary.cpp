...
{
    return lhs.m_brightness == rhs.m_brightness && lhs.m_contrast == rhs.m_contrast
           && lhs.m_gamma == rhs.m_gamma && lhs.m_offset == rhs.m_offset
           && lhs.m_exposure == rhs.m_exposure && lhs.m_lift == rhs.m_lift
           && lhs.m_gain == rhs.m_gain && lhs.m_pivot == rhs.m_pivot
           && lhs.m_saturation == rhs.m_saturation && lhs.m_clampWhite == rhs.m_clampWhite
           && lhs.m_clampBlack == rhs.m_clampBlack && lhs.m_pivotWhite == rhs.m_pivotWhite
...
void GradingPrimary::validate(GradingStyle style) const
{
    ...
    static constexpr double GradingPrimaryMin = GradingPrimaryLowerBound - GradingPrimaryBoundError;

    if (style != GRADING_LIN
        && (m_gamma.m_red < GradingPrimaryMin || m_gamma.m_green < GradingPrimaryMin
            || m_gamma.m_blue < GradingPrimaryMin || m_gamma.m_master < GradingPrimaryMin))
    {
    ...
    if (style == GRADING_LIN
        && (m_contrast.m_red < GradingPrimaryMin || m_contrast.m_green < GradingPrimaryMin
            || m_contrast.m_blue < GradingPrimaryMin || m_contrast.m_master < GradingPrimaryMin))
    {
        std::ostringstream oss;
...
void GradingPrimaryPreRender::update(
    GradingStyle style,
    TransformDirection dir,
    const GradingPrimary & v) noexcept
{
    m_localBypass = v.m_saturation == 1. && v.m_clampBlack == GradingPrimary::NoClampBlack()
    ...
    switch (style)
    {
        case GRADING_LOG:
        {
            ...
            m_localBypass     = m_localBypass && m_isPowerIdentity && m_brightness[0] == 0.f
                            && m_brightness[1] == 0.f && m_brightness[2] == 0.f
        ...
        case GRADING_LIN:
        {
            ...
            m_isPowerIdentity
                = m_contrast[0] == 1.0f && m_contrast[1] == 1.0f && m_contrast[2] == 1.0f;
            m_pivot       = 0.18 * std::pow(2., v.m_pivot);
            m_localBypass = m_localBypass && m_isPowerIdentity && m_exposure[0] == 1.f
                            && m_exposure[1] == 1.f && m_exposure[2] == 1.f && m_offset[0] == 0.f
                            && m_offset[1] == 0.f && m_offset[2] == 0.f;
            break;
        }
        case GRADING_VIDEO:
        {
            ...
            switch (dir)
            {
                case TRANSFORM_DIR_FORWARD:
                {
                    ...
                    m_offset[1]
                        = static_cast<float>(o.m_master + o.m_green + l.m_master + l.m_green);
                    m_offset[2] = static_cast<float>(o.m_master + o.m_blue + l.m_master + l.m_blue);
                    const double slopeDen0
                        = v.m_pivotWhite / gain0 + l.m_master + l.m_red - v.m_pivotBlack;
                    const double slopeDen1
                        = v.m_pivotWhite / gain1 + l.m_master + l.m_green - v.m_pivotBlack;
                    const double slopeDen2
                        = v.m_pivotWhite / gain2 + l.m_master + l.m_blue - v.m_pivotBlack;
                    m_slope[0] = static_cast<float>(
                        (v.m_pivotWhite - v.m_pivotBlack) / (slopeDen0 == 0. ? 1. : slopeDen0));
                    m_slope[1] = static_cast<float>(
                        (v.m_pivotWhite - v.m_pivotBlack) / (slopeDen1 == 0. ? 1. : slopeDen1));
                    m_slope[2] = static_cast<float>(
                        (v.m_pivotWhite - v.m_pivotBlack) / (slopeDen2 == 0. ? 1. : slopeDen2));
                    m_gamma[0] = static_cast<float>(1. / (g.m_master * g.m_red));
                    m_gamma[1] = static_cast<float>(1. / (g.m_master * g.m_green));
                    m_gamma[2] = static_cast<float>(1. / (g.m_master * g.m_blue));
                    break;
                ...
                case TRANSFORM_DIR_INVERSE:
                {
                    ...
                    m_offset[1]
                        = -static_cast<float>(o.m_master + o.m_green + l.m_master + l.m_green);
                    m_offset[2]
                        = -static_cast<float>(o.m_master + o.m_blue + l.m_master + l.m_blue);
                    m_slope[0] = static_cast<float>(
                        (v.m_pivotWhite / gain0 + (l.m_master + l.m_red - v.m_pivotBlack))
            ...
            m_isPowerIdentity = m_gamma[0] == 1.0f && m_gamma[1] == 1.0f && m_gamma[2] == 1.0f;
            m_localBypass     = m_localBypass && m_isPowerIdentity && m_slope[0] == 1.f
                            && m_slope[1] == 1.f && m_slope[2] == 1.f && m_offset[0] == 0.f
                            && m_offset[1] == 0.f && m_offset[2] == 0.f;
            break;
        }
    }
