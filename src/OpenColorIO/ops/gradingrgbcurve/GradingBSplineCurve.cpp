...
void PrepHueCurveData(
    const std::vector<GradingControlPoint> & ctrlPnts,
    ...
    bool isHorizontal)
{
    ...
    for (unsigned i = 0; i < numCtrlPnts; ++i)
    {
        ...
        {
            outCtrlPnts.push_back(
                GradingControlPoint(xval + 1.f, isHorizontal ? yval : yval + 1.f));
        }
    ...
    std::vector<float> & coefsC)
{
    ...
    for (unsigned i = 0; i < numCtrlPnts - 1; ++i)
    {
        ...
        else
        {
            ...
            coefsC.push_back(
                p0.m_y + slopes[i] * (ksi - p0.m_x) + 0.5f * eta * (ksi - p0.m_x) * (ksi - p0.m_x));
    ...
    bool isHorizontal)
{
    ...
    else // Diagonal curves except hue-hue (LvL and SvS).
    {
        ...
        slopes.push_back(
            std::max(
                minSlope,
                0.5f * (3.f * secantSlope[numCtrlPnts - 2] - slopes[numCtrlPnts - 2])));
    ...
    std::vector<float> & slopes)
{
    ...
    slopes.push_back(
        std::max(0.01f, 0.5f * (3.f * secantSlope[numCtrlPnts - 2] - slopes[numCtrlPnts - 2])));
    ...
    std::vector<float> & knots)
{
    ...
    while (j < n)
    {
        if (ctrlPnts[i].m_x != knots[j])
        {
            ...
            const float yi_pl1 = ctrlPnts[i + 1].m_y;
            const float s_bar
                = (2.f * (yi_pl1 - yi) - (ksi - xi) * slopes[i] - (xi_pl1 - ksi) * slopes[i + 1])
                  / (xi_pl1 - xi);
            if (s_bar < 0.f)
            {
                adjustment_done = true;
                float secant    = (yi_pl1 - yi) / (xi_pl1 - xi);
                const float blend_slope
...
GradingBSplineCurveRcPtr GradingBSplineCurve::Create(
    std::initializer_list<GradingControlPoint> values)
{
...
GradingBSplineCurveImpl::GradingBSplineCurveImpl(size_t size)
    ...
    , m_slopesArray(size, 0.f)
    , m_splineType(B_SPLINE)
{
}
...
GradingBSplineCurveImpl::GradingBSplineCurveImpl(
    const std::vector<GradingControlPoint> & controlPoints)
    : m_controlPoints(controlPoints)
    , m_slopesArray(controlPoints.size(), 0.f)
    , m_splineType(B_SPLINE)
{
}
...
void GradingBSplineCurveImpl::validateIndex(size_t index) const
{
    ...
    if (index >= numPoints)
    {
        ...
        oss << "There are '" << numPoints << "' control points. '" << index
            << "' is out of bounds.";
...
void GradingBSplineCurveImpl::validate() const
{
    ...
    if (m_splineType == B_SPLINE || m_splineType == DIAGONAL_B_SPLINE
        || m_splineType == HUE_HUE_B_SPLINE)
    {
        ...
        for (size_t i = 0; i < numPoints; ++i)
        {
            ...
            {
                std::ostringstream oss;
                oss << "Control point at index " << i << " has a y coordinate '" << y
                    << "' that is ";
    ...
    // Don't allow only x values of 0 and 1 for periodic curves (since they are essentially only one
    // point).
    if (numPoints == 2)
    {
        // Periodic curve types only.
        if (m_splineType == PERIODIC_1_B_SPLINE || m_splineType == PERIODIC_0_B_SPLINE
            || m_splineType == HUE_HUE_B_SPLINE)
        {
...
bool GradingBSplineCurveImpl::isIdentity() const
{
    ...
    if (m_splineType == DIAGONAL_B_SPLINE || m_splineType == B_SPLINE
        || m_splineType == HUE_HUE_B_SPLINE)
    {
        isIdentity = std::all_of(
            m_controlPoints.cbegin(),
...
//------------------------------------------------------------------------------------------------
//
void GradingBSplineCurveImpl::computeKnotsAndCoefsForRGBCurve(KnotsCoefs & knotsCoefs, int curveIdx)
    const
{
    ...
    else
    {
        ...
        std::copy(
            coefsB.begin(),
            coefsB.end(),
            knotsCoefs.m_coefsArray.begin() + numCoefs + coefsSize);
        std::copy(
            coefsC.begin(),
            coefsC.end(),
            knotsCoefs.m_coefsArray.begin() + numCoefs + coefsSize * 2);
...
//------------------------------------------------------------------------------------------------
//
void GradingBSplineCurveImpl::computeKnotsAndCoefsForHueCurve(
    KnotsCoefs & knotsCoefs,
    int curveIdx,
...
//------------------------------------------------------------------------------------------------
//
void GradingBSplineCurveImpl::computeKnotsAndCoefs(
    KnotsCoefs & knotsCoefs,
    int curveIdx,
    bool drawCurveOnly) const
{
...
//------------------------------------------------------------------------------------------------
//
void GradingBSplineCurveImpl::AddShaderEvalFwd(
    GpuShaderText & st,
...
//------------------------------------------------------------------------------------------------
//
void GradingBSplineCurveImpl::AddShaderEvalRev(
    GpuShaderText & st,
    const std::string & knotsOffsets,
    const std::string & coefsOffsets,
    const std::string & knots,
...
//------------------------------------------------------------------------------------------------
//
void GradingBSplineCurveImpl::AddShaderEvalRevHue(
    GpuShaderText & st,
    const std::string & knotsOffsets,
    const std::string & coefsOffsets,
    const std::string & knots,
    const std::string & coefs)
{
    ...
    st.newLine() << "  curve_x = (curveIdx == 7) ? curve_x + " << knots
