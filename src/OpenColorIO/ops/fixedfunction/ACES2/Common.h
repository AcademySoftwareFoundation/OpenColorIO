...
inline constexpr float to_degrees(const float v)
{
    return v;
}
inline float from_degrees(const float v)
{
    return wrap_to_hue_limit(v);
}
inline constexpr float to_radians(const float v)
{
    return PI * v / 180.0f;
};
inline float _from_radians(const float v)
{
    return _wrap_to_hue_limit(180.0f * v / PI);
}; // v needs to be wrapped already
inline float from_radians(const float v)
{
    return wrap_to_hue_limit(180.0f * v / PI);
};
/*
...
struct TableBase
{
    ...
    static constexpr unsigned int total_size
        = nominal_size + _TABLE_ADDITION_LOWER_ENTRIES + _TABLE_ADDITION_UPPER_ENTRIES;
    ...
    inline unsigned int hue_position_in_uniform_table(float wrapped_hue) const
    {
        if (hue_limit == float(nominal_size)) // TODO C++ 17 if constexpr
            ...
            return static_cast<unsigned int>(
                wrapped_hue / hue_limit
                * float(nominal_size)); // TODO: can we use the 'lost' fraction for the lerps?
    }
