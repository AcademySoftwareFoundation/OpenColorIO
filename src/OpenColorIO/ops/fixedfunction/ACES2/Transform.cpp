...
unsigned int lookup_hue_interval(
    float h,
    const Table1D & hues,
    const std::array<int, 2> & hue_linearity_search_range)
{
    ...
    unsigned int i_lo = std::max(
        int(hues.lower_wrap_index),
        int(i) + hue_linearity_search_range[0]); // Should be nominal not lower_wrap?
    unsigned int i_hi
        = std::min(int(hues.upper_wrap_index), int(i) + hue_linearity_search_range[1]);
    ...
    i_hi = std::max(
        1U,
        i_hi); // TODO: should not be needed if we initialise with the correct lo and hi
...
float chroma_compress_norm(float cos_hr1, float sin_hr1, float chroma_compress_scale)
{
    ...
    alignas(AVX_ALIGNMENT) const float trig_angles_hr[8]
        = {cos_hr1, cos_hr2, cos_hr3, 0.0f, sin_hr1, sin_hr2, sin_hr3, 1.0f};
    alignas(AVX_ALIGNMENT) static constexpr float weights[8]
        = {// TODO: investigate reordering of the entries so we are summing equal magnitude values
           // first?
           11.34072f,
           16.46899f,
           7.88380f,
...
#else
    */
    const float M = weights[0] * trig_angles_hr[0] + weights[1] * trig_angles_hr[1]
                    + weights[2] * trig_angles_hr[2] + weights[4] * trig_angles_hr[4]
                    + weights[5] * trig_angles_hr[5] + weights[6] * trig_angles_hr[6] + weights[7];
    /*
#endif
    */
...
inline float toe_fwd(float x, float limit, float k1_in, float k2_in)
{
    ...
    return 0.5f
           * (minus_b
              + sqrt(minus_b * minus_b + 4.f * minus_ac)); // a is 1.0, mins_b squared == b^2
}
...
template <bool inverse> inline float aces_tonescale(const float Y_in, const ToneScaleParams & pt)
{
    if (inverse)
    {
        ...
        const float Z         = std::max(
            ...
            std::min(
                pt.inverse_limit,
                Y_ts_norm)); // TODO: could eliminate max in the context of the full tonescale
        const float f = (Z + sqrt(Z * (4.f * pt.t_1 + Z))) / 2.f;
        const float Y
            = pt.s_2
              / (powf((pt.m_2 / f), (1.f / pt.g))
                 - 1.f); // TODO: investigate precomputing reciprocal m_2  and a negative power? may
                         // swap a division for a multiply? powf(pt.m_2_recip * f, (-1.f / pt.g))
        return Y;
    }

    const float f    = pt.m_2 * powf(Y_in / (Y_in + pt.s_2), pt.g);
    const float Y_ts = std::max(0.f, f * f / (f + pt.t_1))
                       * pt.n_r; // max prevents -ve values being output also handles division by
                                 // zero possibility // TODO: can we eliminate the n_r scaling?
                                 // should it be reference_luminance
    return Y_ts;
}

template <bool inverse>
inline float tonescale(
    const float J,
    const JMhParams & p,
    const ToneScaleParams & pt) // TODO: consider computing tonescale from and to A rather than J to
                                // avoid extra pow() calls
{
...
f3 chroma_compress_fwd(
    const f3 & JMh,
    const float J_ts,
    const float Mnorm,
    const ResolvedSharedCompressionParameters & pr,
    const ChromaCompressParams & pc)
{
    const float J = JMh[0];
    const float M = JMh[1];
    const float h = JMh[2];
    ...
    if (M != 0.0)
    {
        ...
        M_cp = M_cp / Mnorm;
        M_cp = limit
               - toe_fwd(limit - M_cp, limit - 0.001f, snJ * pc.sat, sqrt(nJ * nJ + pc.sat_thr));
...
JMhParams init_JMhParams(const Primaries & prims)
{
    ...
    const m33f cone_response_to_Aab = mult_f33_f33(
        scale_f33(Identity_M33, f3_from_f(cam_nl_scale)),
        base_cone_response_to_Aab); // TODO: this scale maybe ill conditioning the matrix
    const float A_w   = cone_response_to_Aab[0] * RGB_AW[0] + cone_response_to_Aab[1] * RGB_AW[1]
    ...
    // TODO: evaluate the condition number of the below matrices and consider extracting a power of
    // 2 scale out of them may improve noise behaviour
    ...
    const m33f MATRIX_RGB_to_CAM16 = mult_f33_f33(
        RGBtoRGB_f33(prims, CAM16::primaries),
        scale_f33(Identity_M33, f3_from_f(reference_luminance)));
    const m33f MATRIX_RGB_to_CAM16_c
    ...
    const m33f MATRIX_cone_response_to_Aab = {
        cone_response_to_Aab[0] / A_w,
        cone_response_to_Aab[1] / A_w,
        cone_response_to_Aab[2] / A_w,
        cone_response_to_Aab[3] * 43.f * surround[2],
        cone_response_to_Aab[4] * 43.f * surround[2],
        cone_response_to_Aab[5] * 43.f * surround[2],
        cone_response_to_Aab[6] * 43.f * surround[2],
        cone_response_to_Aab[7] * 43.f * surround[2],
        cone_response_to_Aab[8] * 43.f * surround[2],
    };
...
inline f3 generate_unit_cube_cusp_corners(const unsigned int corner)
{
    ...
    return {
        float(static_cast<unsigned int>(((corner + 1) % cuspCornerCount) < 3)),
        float(static_cast<unsigned int>(((corner + 5) % cuspCornerCount) < 3)),
        float(static_cast<unsigned int>(((corner + 3) % cuspCornerCount) < 3))};
}

void build_limiting_cusp_corners_tables(
    std::array<f3, totalCornerCount> & RGB_corners,
    std::array<f3, totalCornerCount> & JMh_corners,
    const JMhParams & params,
    const float peakLuminance)
{
    ...
    {
        temp_RGB_corners[i]
            = mult_f_f3(peakLuminance / reference_luminance, generate_unit_cube_cusp_corners(i));
    ...
    const float maximum_source)
{
    ...
    // This is done by scaling the unit corners converting to JMh until the J value is near the
    // limitJ As an optimisation we use the equivalent Achromatic value to search for the J value
    // and avoid the non-linear transform during the search Strictly speaking we should only need to
    // find the R, G and  B "corners" as the reach is unbounded and as such does not form a cube,
    // but is formed by the transformed 3 lower planes of the cube and the plane at J = limitJ
    std::array<f3, cuspCornerCount> temp_JMh_corners;
    ...
    // Rotate entries placing lowest at [1] (not [0]) // TODO: could use std::rotate_copy or even
    // the ranges vs in C++20
    for (unsigned int i = 0; i != cuspCornerCount; ++i)
    {
        JMh_corners[i + 1] = temp_JMh_corners[(i + min_index) % cuspCornerCount];
    }
...
unsigned int extract_sorted_cube_hues(
    std::array<float, max_sorted_corners> & sorted_hues,
    const std::array<f3, totalCornerCount> & reach_JMh,
    const std::array<f3, totalCornerCount> & display_JMh)
{
...
void build_hue_sample_interval(
...
void build_hue_table(
    Table1D & hue_table,
    const std::array<float, max_sorted_corners> & sorted_hues,
    const unsigned int unique_hues)
{
    ...
    unsigned int min_index
        = sorted_hues[0] == 0.0f ? 0 : 1; // Ensure we can always sample at 0.0 hue
    for (unsigned int hue_idx = 0; hue_idx != unique_hues; ++hue_idx)
    {
        // BUG: "hue_table.size - 1" will fail if we have multiple hues mapping near the top of the
        // table
        unsigned int nominal_idx = std::min(
            std::max(
                static_cast<unsigned int>(std::round(sorted_hues[hue_idx] * ideal_spacing)),
                min_index),
            hue_table.nominal_size - 1);
    ...
    build_hue_sample_interval(
        hue_table.nominal_size - total_samples,
        sorted_hues[i - 1],
        hue_limit,
        hue_table,
        total_samples + 1);
    ...
    hue_table[hue_table.upper_wrap_index + 1]
        = hue_table[hue_table.first_nominal_index + 1] + hue_limit;
}

std::array<float, 2> find_display_cusp_for_hue(
    float hue,
    const std::array<f3, totalCornerCount> & RGB_corners,
    const std::array<f3, totalCornerCount> & JMh_corners,
    const JMhParams & params,
    std::array<float, 2> & previous)
{
    // This works by finding the required line segment between two of the XYZ cusp corners, then
    // binary searching along the line calculating the JMh of points along the line till we find the
    // required value. All values on the line segments are valid cusp locations.
    ...
    float sample_t;
    float lower_t = (upper_corner == previous[0])
                        ? previous[1]
    ...
    // against the appropriate corner to make sure we are still in the expected range between the
    // lower and upper corner hue limits
    ...
    const JMhParams & params)
{
    ...
    for (unsigned int i = output_table.first_nominal_index; i != output_table.upper_wrap_index; ++i)
    {
        ...
        const std::array<float, 2> JM
            = find_display_cusp_for_hue(hue, RGB_corners, JMh_corners, params, previous);
    ...
    output_table[output_table.lower_wrap_index][0]
        = output_table[output_table.last_nominal_index][0];
    output_table[output_table.lower_wrap_index][1]
        = output_table[output_table.last_nominal_index][1];
    output_table[output_table.lower_wrap_index][2] = hue_table[hue_table.lower_wrap_index];
    output_table[output_table.upper_wrap_index][0]
        = output_table[output_table.first_nominal_index][0];
    output_table[output_table.upper_wrap_index][1]
        = output_table[output_table.first_nominal_index][1];
    output_table[output_table.upper_wrap_index][2] = hue_table[hue_table.upper_wrap_index];
    output_table[output_table.upper_wrap_index + 1][0]
        = output_table[output_table.first_nominal_index + 1][0];
    output_table[output_table.upper_wrap_index + 1][1]
        = output_table[output_table.first_nominal_index + 1][1];
    output_table[output_table.upper_wrap_index + 1][2] = hue_table[hue_table.upper_wrap_index + 1];
    return output_table;
}

Table3D make_uniform_hue_gamut_table(
    const JMhParams & reach_params,
    const JMhParams & params,
    float peakLuminance,
    float forward_limit,
    const SharedCompressionParameters & sp,
    Table1D & hue_table)
{
    // The principal here is to sample the hues as uniformly as possible, whilst ensuring we sample
    // the corners of the limiting gamut and the reach primaries at limit J Max
    //
    // The corners are calculated then the hues are extracted and merged to form a unique sorted hue
    // list We then build the hue table from the list, those hues are then used to compute the JMh
    // od the limiting gamut cusp.
...
Table1D make_reach_m_table(const JMhParams & params, const float limit_J_max)
{
    ...
    gamutReachTable[gamutReachTable.lower_wrap_index]
        = gamutReachTable[gamutReachTable.last_nominal_index];
    gamutReachTable[gamutReachTable.upper_wrap_index]
        = gamutReachTable[gamutReachTable.first_nominal_index];
    gamutReachTable[gamutReachTable.upper_wrap_index + 1]
        = gamutReachTable[gamutReachTable.first_nominal_index + 1];
...
inline float
get_focus_gain(float J, float analytical_threshold, float limit_J_max, float focus_dist)
{
    ...
    if (J > analytical_threshold)
    {
        ...
        float gain_adjustment
            = log10((limit_J_max - analytical_threshold) / std::max(0.0001f, limit_J_max - J));
...
float solve_J_intersect(
    ...
    float focusJ,
    float maxJ,
    float slope_gain) // TODO: eval placing maxJ as last patrameter?
{
...
inline float compute_compression_vector_slope(
    const float intersectJ,
    const float focusJ,
    const float limitJmax,
    const float slope_gain)
{
    const float direction_scaler
        = (intersectJ < focusJ) ? intersectJ : (limitJmax - intersectJ); // TODO < vs <=
    return direction_scaler * (intersectJ - focusJ) / (focusJ * slope_gain);
}

inline float estimate_line_and_boundary_intersection_M(
    const float J_axis_intersect,
    const float slope,
    ...
    const float J_intersection_reference)
{
    ...
    // Approximate as we do not want to iteratively solve intersection of a straight line and an
    // exponential

    // We calculate a shifted intersection from the original intersection using the inverse of the
    // exponential and the provided reference
...
template <bool invert> inline float reinhard_remap(const float scale, const float nd)
{
    if (invert)
    {
        if (nd >= 1.0f) // TODO: given remap_M already tests against proportion do we need this
                        // asymptote test
            return scale;
        else
...
f3 compressGamut(
    ...
    const ACES2::GamutCompressParams & p,
    const HueDependantGamutParams hdp)
{
    const float J = JMh[0];
    const float M = JMh[1];
    const float h = JMh[2];
    ...
    const float slope_gain
        = get_focus_gain(Jx, hdp.analytical_threshold, sr.limit_J_max, p.focus_dist);
    const float J_intersect_source
        = solve_J_intersect(J, M, hdp.focusJ, sr.limit_J_max, slope_gain);
    ...
    const float J_intersect_cusp
        = solve_J_intersect(hdp.JMcusp[0], hdp.JMcusp[1], hdp.focusJ, sr.limit_J_max, slope_gain);
    ...
    const float reachBoundaryM = estimate_line_and_boundary_intersection_M(
        J_intersect_source,
        gamut_slope,
        sr.model_gamma_inv,
        sr.limit_J_max,
    ...
    const f3 JMcompressed{J_intersect_source + remapped_M * gamut_slope, remapped_M, h};
    return JMcompressed;
}
...
HueDependantGamutParams init_HueDependantGamutParams(
    const float hue,
    const ResolvedSharedCompressionParameters & sr,
    const GamutCompressParams & p)
{
...
std::array<testData, gamma_test_count> generate_gamma_test_data(
    const f2 & JMcusp,
    float hue,
    float limit_J_max,
    float mid_J,
    ...
    std::generate(
        ...
        [JMcusp,
         ...
         testIndex]() mutable {
            ...
            const float slope_gain
                = get_focus_gain(testJ, analytical_threshold, limit_J_max, focus_dist);
            const float J_intersect_source
                = solve_J_intersect(testJ, JMcusp[1], focusJ, limit_J_max, slope_gain);
            testData result = {
                {testJ, JMcusp[1], hue},
                J_intersect_source,
    ...
    const JMhParams & limitJMhParams)
{
    ...
    {
        const float approxLimit_M = find_gamut_boundary_intersection(
            JMcusp,
            limit_J_max,
            topGamma_inv,
...
void make_upper_hull_gamma(
    ...
    const JMhParams & limitJMhParams)
{
    for (unsigned int i = gamutCuspTable.first_nominal_index; i != gamutCuspTable.upper_wrap_index;
         ++i)
    {
        const float hue = hue_table[i];
        ...
        std::array<testData, gamma_test_count> data
            = generate_gamma_test_data(JMcusp, hue, limit_J_max, mid_J, focus_dist);

        // TODO: calculate best fitting inverse gamma directly rather than reciprocating it in the
        // loop
        // TODO: adjacent found gamma values typically fall close to each other could initialise the
        // search range
        //       with values near to speed up searching. Note that discrepancies do occur at/near
        //       corners of gamut
        ...
        auto gamma_fit_predicate
            = [JMcusp, data, peakLuminance, limit_J_max, lower_hull_gamma_inv, limitJMhParams](
                  float gamma) {
                  return evaluate_gamma_fit(
                      JMcusp,
                      data,
                      1.0f / gamma,
                      peakLuminance,
    ...
    gamutCuspTable[gamutCuspTable.lower_wrap_index][2]
        = gamutCuspTable[gamutCuspTable.last_nominal_index][2];
    gamutCuspTable[gamutCuspTable.upper_wrap_index][2]
        = gamutCuspTable[gamutCuspTable.first_nominal_index][2];
    gamutCuspTable[gamutCuspTable.upper_wrap_index + 1][2]
        = gamutCuspTable[gamutCuspTable.first_nominal_index + 1][2];
}
...
ToneScaleParams init_ToneScaleParams(float peakLuminance)
{
    ...
    ToneScaleParams TonescaleParams
        = {n, n_r, g, t_1, c_t, s_2, u_2, m_2, forward_limit, inverse_limit, log_peak};

    return TonescaleParams;
}

SharedCompressionParameters init_SharedCompressionParams(
    float peakLuminance,
    const JMhParams & inputJMhParams,
    const JMhParams & reachParams)
{
    ...
    SharedCompressionParameters params
        = {limit_J_max, model_gamma_inv, make_reach_m_table(reachParams, limit_J_max)};
    return params;
}

ResolvedSharedCompressionParameters resolve_CompressionParams(
    float hue,
    const SharedCompressionParameters & p)
{
    ResolvedSharedCompressionParameters params
        = {p.limit_J_max, p.model_gamma_inv, reach_m_from_table(hue, p.reach_m_table)};
    return params;
}

ChromaCompressParams init_ChromaCompressParams(
    float peakLuminance,
    const ToneScaleParams & tsParams)
{
    // Calculated chroma compress variables
    // TODO: name these magic constants
    const float compr
        = chroma_compress + (chroma_compress * chroma_compress_fact) * tsParams.log_peak;
    const float sat
        = std::max(0.2f, chroma_expand - (chroma_expand * chroma_expand_fact) * tsParams.log_peak);
    ...
    ChromaCompressParams params = {sat, sat_thr, compr, chroma_compress_scale};
    return params;
}

std::array<int, 2> determine_hue_linearity_search_range(const Table3D & gamutCuspTable)
{
    // This function searches through the hues looking for the largest deviations from a linear
    // distribution We can then use this to initialise the binary search range to something smaller
    // than the full one to reduce the number of lookups per hue lookup from ~ceil(log2(table size))
    // to ~ceil(log2(range)) during image rendering.

    // TODO: Padding values are a quick hack to ensure the range encloses the needed range, left as
    // an exersize for the reader to fully reason if these values could be smaller, probably best
    // done the closer the hue distribution comes to linear as the overhead becomes a potentially
    // greater issue
    ...
    for (unsigned int i = gamutCuspTable.first_nominal_index; i != gamutCuspTable.upper_wrap_index;
         ++i)
    {
        const unsigned int pos = gamutCuspTable.nominal_hue_position_in_uniform_table(
            gamutCuspTable[i][2]); // TODO: compute from hue table less cache pressure?
        const int delta = int(i) - int(pos);
        hue_linearity_search_range[0]
            = std::min(hue_linearity_search_range[0], delta + lower_padding);
        hue_linearity_search_range[1]
            = std::max(hue_linearity_search_range[1], delta + upper_padding);

        // std::cout << i << " " << pos << " " << delta << " " << gamutCuspTable[i][0] << " " <<
        // gamutCuspTable[i][1] << " " << gamutCuspTable[i][2] << "\n";
    }
    // std::cout << "search range " << hue_linearity_search_range[0] << " " <<
    // hue_linearity_search_range[1] << "\n";
    return hue_linearity_search_range;
}

GamutCompressParams init_GamutCompressParams(
    float peakLuminance,
    const JMhParams & inputJMhParams,
    const JMhParams & limitJMhParams,
    const ToneScaleParams & tsParams,
    const SharedCompressionParameters & shParams,
    const JMhParams & reachParams)
{
    ...
    const float focus_dist
        = focus_distance + focus_distance * focus_distance_scaling * tsParams.log_peak;
    const float lower_hull_gamma_inv
        = 1.0f / (1.14f + 0.07f * tsParams.log_peak); // TODO: name these magic constants

    GamutCompressParams params;
    params.mid_J                = mid_J;
    params.focus_dist           = focus_dist;
    params.lower_hull_gamma_inv = lower_hull_gamma_inv;
    params.gamut_cusp_table     = make_uniform_hue_gamut_table(
        reachParams,
        limitJMhParams,
        peakLuminance,
        ...
        shParams,
        params.hue_table);
    params.hue_linearity_search_range
        = determine_hue_linearity_search_range(params.gamut_cusp_table);
    make_upper_hull_gamma(
        params.hue_table,
        params.gamut_cusp_table,
        peakLuminance,
        shParams.limit_J_max,
        mid_J,
        focus_dist,
        lower_hull_gamma_inv,
        limitJMhParams); // TODO: mess of parameters
    return params;
}
