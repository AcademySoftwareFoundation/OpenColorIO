...
template <typename T> class Renderer_LIN_TO_PQ : public OpCPU
{
public:
...
template <typename T> class Renderer_PQ_TO_LIN : public OpCPU
{
public:
...
#if OCIO_USE_SSE2
template <bool FAST_POWER> class Renderer_LIN_TO_PQ_SSE : public OpCPU
{
public:
...
template <bool FAST_POWER> class Renderer_PQ_TO_LIN_SSE : public OpCPU
{
public:
...
Renderer_ACES_OutputTransform20::Renderer_ACES_OutputTransform20(
    ConstFixedFunctionOpDataRcPtr & data)
    : OpCPU()
{
...
void Renderer_ACES_OutputTransform20::fwd(const void * inImg, void * outImg, long numPixels) const
{
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        const ACES2::ResolvedSharedCompressionParameters rp
            = resolve_CompressionParams(JMh[2], m_s);
        const float h_rad   = ACES2::to_radians(JMh[2]);
        const float cos_hr1 = std::cos(h_rad);
        const float sin_hr1 = std::sin(h_rad);
        const float Mnorm
            = ACES2::chroma_compress_norm(cos_hr1, sin_hr1, m_c.chroma_compress_scale);
...
void Renderer_ACES_OutputTransform20::inv(const void * inImg, void * outImg, long numPixels) const
{
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        const ACES2::ResolvedSharedCompressionParameters rp
            = resolve_CompressionParams(compressedJMh[2], m_s);
...
Renderer_ACES_TONESCALE_COMPRESS_20::Renderer_ACES_TONESCALE_COMPRESS_20(
    ConstFixedFunctionOpDataRcPtr & data)
    : OpCPU()
{
...
void Renderer_ACES_TONESCALE_COMPRESS_20::apply(const void * inImg, void * outImg, long numPixels)
    const
{
    if (m_fwd)
    {
...
void Renderer_ACES_TONESCALE_COMPRESS_20::fwd(const void * inImg, void * outImg, long numPixels)
    const
{
    const float * in = (const float *)inImg;
    float * out      = (float *)outImg;
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        const float Mnorm
            = ACES2::chroma_compress_norm(cos_hr1, sin_hr1, m_c.chroma_compress_scale);
        const ACES2::ResolvedSharedCompressionParameters rp
            = resolve_CompressionParams(normalised_hue, m_s);
        const float J_ts = ACES2::tonescale_fwd(in[0], m_p, m_t);
        const ACES2::f3 JMh
            = ACES2::chroma_compress_fwd({in[0], in[1], normalised_hue}, J_ts, Mnorm, rp, m_c);
...
void Renderer_ACES_TONESCALE_COMPRESS_20::inv(const void * inImg, void * outImg, long numPixels)
    const
{
    const float * in = (const float *)inImg;
    float * out      = (float *)outImg;
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        const ACES2::f3 JMh
            = ACES2::chroma_compress_inv({in[0], in[1], normalised_hue}, J, Mnorm, rp, m_c);
...
Renderer_ACES_GAMUT_COMPRESS_20::Renderer_ACES_GAMUT_COMPRESS_20(
    ConstFixedFunctionOpDataRcPtr & data)
    : OpCPU()
{
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        if (funcStyle == FixedFunctionOpData::HSY_LIN_TO_RGB)
        {
            ...
            else
            {
                ...
                const float b       = distRgb * loGain * (1.f - alpha) * (k + 3.f * luma)
                                      + distRgb * alpha - sat * (sumRgb - 3.f * luma);
...
template <> __m128 Renderer_PQ_TO_LIN_SSE<true>::myPower(__m128 x, __m128 exp)
{
...
template <> __m128 Renderer_PQ_TO_LIN_SSE<false>::myPower(__m128 x, __m128 exp)
{
...
template <bool FAST_POWER>
void Renderer_PQ_TO_LIN_SSE<FAST_POWER>::apply(const void * inImg, void * outImg, long numPixels)
    const
{
    using namespace ST_2084;
    const float * in = (const float *)inImg;
    ...
    for (long idx = 0; idx < numPixels; ++idx, in += 4, out += 4)
    {
        ...
        __m128 nits100_signed
            = _mm_or_ps(_mm_and_ps(abs_rgb_mask, nits100), _mm_andnot_ps(abs_rgb_mask, v));
...
// All platforms support ssePower().
template <> __m128 Renderer_LIN_TO_PQ_SSE<true>::myPower(__m128 x, __m128 exp)
{
    return ssePower(x, exp);
}
...
// now.
template <> __m128 Renderer_LIN_TO_PQ_SSE<false>::myPower(__m128 x, __m128 exp)
{
    return _mm_pow_ps(x, exp);
}
...
template <bool FAST_POWER>
void Renderer_LIN_TO_PQ_SSE<FAST_POWER>::apply(const void * inImg, void * outImg, long numPixels)
    const
{
    using namespace ST_2084;
    const float * in = (const float *)inImg;
...
void Renderer_LIN_TO_DOUBLE_LOG::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        for (int ch = 0; ch < 3; ++ch)
        {
            ...
            // Linear segment may not exist or be valid. Thus we include the break points in the log
            // segments.
            if (x <= m_break1)
            {
                x = m_logSeg1.logSlope * std::log(m_logSeg1.linSlope * x + m_logSeg1.linOff)
                    + m_logSeg1.logOff;
            }
            else if (x < m_break2)
            {
            ...
            {
                x = m_logSeg2.logSlope * std::log(m_logSeg2.linSlope * x + m_logSeg2.linOff)
                    + m_logSeg2.logOff;
            }
    ...
    : Renderer_LIN_TO_DOUBLE_LOG(data)
{
    ...
    m_break1Log = m_logSeg1.logSlope * std::log(m_logSeg1.linSlope * m_break1 + m_logSeg1.linOff)
                  + m_logSeg1.logOff;
    m_break2Log = m_logSeg2.logSlope * std::log(m_logSeg2.linSlope * m_break2 + m_logSeg2.linOff)
                  + m_logSeg2.logOff;
}
...
void Renderer_DOUBLE_LOG_TO_LIN::apply(const void * inImg, void * outImg, long numPixels) const
{
    ...
    for (long idx = 0; idx < numPixels; ++idx)
    {
        ...
        for (int ch = 0; ch < 3; ++ch)
        {
            ...
            if (y <= m_break1Log)
            {
                y = (std::exp((y - m_logSeg1.logOff) / m_logSeg1.logSlope) - m_logSeg1.linOff)
                    / m_logSeg1.linSlope;
            }
            ...
            {
                y = (std::exp((y - m_logSeg2.logOff) / m_logSeg2.logSlope) - m_logSeg2.linOff)
                    / m_logSeg2.linSlope;
            }
