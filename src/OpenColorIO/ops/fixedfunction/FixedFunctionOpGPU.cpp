    ...
    const ACES2::Table1D & table)
{
    ...
    ss.newLine() << ss.floatDecl("i_lo") << " = i_base + " << ss.floatKeyword() << "("
                 << table.base_index << ");";
    ...
    if (dimensions == GpuShaderDesc::TEXTURE_1D)
    {
        ss.newLine() << ss.floatDecl("lo") << " = "
                     << ss.sampleTex1D(
                            name,
                            "(i_lo + 0.5) / " + ss.floatKeyword() + " ("
                                + std::to_string(table.total_size) + ")")
                     << ".r;";
        ss.newLine() << ss.floatDecl("hi") << " = "
                     << ss.sampleTex1D(
                            name,
                            "(i_hi + 0.5) / " + ss.floatKeyword() + " ("
                                + std::to_string(table.total_size) + ")")
                     << ".r;";
    }
    else
    ...
    const ACES2::GamutCompressParams & g)
{
    ...
    if (dimensions == GpuShaderDesc::TEXTURE_1D)
    {
        ss.newLine() << ss.float3Decl("lo") << " = "
                     << ss.sampleTex1D(
                            name,
                            std::string("(" + ss.floatKeyword() + "(i_hi) - 1.0 + 0.5) / ")
                                + ss.floatKeyword() + "("
        ...
        ss.newLine() << ss.float3Decl("hi") << " = "
                     << ss.sampleTex1D(
                            name,
                            std::string("(" + ss.floatKeyword() + "(i_hi) + 0.5) / ")
                                + ss.floatKeyword() + "("
                                + std::to_string(g.gamut_cusp_table.total_size) + ")")
                     << ".rgb;";
    }
