    ...
    const FixedFunctionOpData::Params & params)
{
    ...
    ss.newLine() << ss.floatDecl("J") << " = 100.f * pow(A / " << p.A_w << ", 0.59f * " << p.z << ");";

    ss.newLine() << ss.floatDecl("M") << " = (J == 0.f) ? 0.f : 43.f * 0.9f * sqrt(a * a + b * b);";

    ss.newLine() << ss.floatDecl("h") << " = " << ss.atan2("b" , "a") << " * 180.f / 3.14159265358979f;";
    ...
    const FixedFunctionOpData::Params & params)
{
    ...
    ss.newLine() << ss.floatDecl("scale") << " = " << pxl << ".g / (43.f * 0.9f);";
    ss.newLine() << ss.floatDecl("A") << " = " << p.A_w << " * pow(" << pxl << ".r / 100.f, 1.f / (0.59f * " << p.z << "));";
    ...
    const FixedFunctionOpData::Params & params)
{
    ...
    ss.newLine() << ss.floatDecl("A") << " = " << p.A_w_J << " * pow(abs(J) / 100.f, 1.f / (0.59f * " << p.z << "));";
    ...
    ss.newLine() << ss.floatDecl("J_ts") << " = sign(Y_ts) * 100.f * pow(((400.f * F_L_Y) / (27.13f + F_L_Y)) / " << p.A_w_J << ", 0.59f * " << p.z << ");";
    ...
    const FixedFunctionOpData::Params & params)
{
    ...
    ss.newLine() << ss.floatDecl("A") << " = " << p.A_w_J << " * pow(abs(J_ts) / 100.f, 1.f / (0.59f * " << p.z << "));";
    ...
    ss.newLine() << ss.floatDecl("J") << " = sign(Y) * 100.f * pow(((400.f * F_L_Y) / (27.13f + F_L_Y)) / " << p.A_w_J << ", 0.59f * " << p.z << ");";
    ...
    const ACES2::GamutCompressParams & g)
{
    ...
    ss.newLine() << "hwrap = hwrap - floor(hwrap / 360.f) * 360.f;";
    ss.newLine() << "hwrap = (hwrap < 0.0f) ? hwrap + 360.f : hwrap;";
    ss.newLine() << ss.floatDecl("i") << " = floor(hwrap + " << g.gamut_cusp_table.base_index << ");";
    ...
    {
        ss.newLine() << ss.floatDecl("hcur") << " = " << ss.sampleTex2D(name, std::string("vec2((i + 0.5f) / ") + std::to_string(g.gamut_cusp_table.total_size) + std::string(", 0.5f)")) << ".b;";
    }
    ...
    ss.dedent();
    ss.newLine() << "}";
    ss.newLine() << "i = floor((i_lo + i_hi) / 2.f);";
    ...
    const ACES2::GamutCompressParams & g)
{
    ...
    ss.newLine() << ss.floatDecl("thr") << " = " << ss.lerp("cuspJ", std::to_string(g.limit_J_max), std::to_string(0.3f)) << ";";

    ss.newLine() << "if (J > thr)";
    ss.newLine() << "{";
    ss.indent();
    ss.newLine() << ss.floatDecl("gain") << " = ( " << g.limit_J_max << " - thr) / max(0.0001f, (" << g.limit_J_max << " - min(" << g.limit_J_max << ", J)));";
    ss.newLine() << "return pow(log(gain)/log(10.f), 1.f / 0.55f) + 1.f;";
    ss.dedent();
    ss.newLine() << "}";
    ss.newLine() << "else";
    ss.newLine() << "{";
    ...
    const std::string & solveJIntersectName)
{
    ...
    ss.newLine() << ss.float2Decl("JM_cusp") << " = " << ss.float2Const("JM_cusp_in.r", "JM_cusp_in.g * (1.f + 0.27f * 0.12f)") << ";";
    ...
    ss.newLine() << ss.floatDecl("s") << " = 0.12f;";
    ...
    const std::string & solveJIntersectName)
{
    ...
    ss.newLine() << ss.float3Keyword() << " " << name << "(float J, float M, float h, " << ss.float2Keyword() << " JMcusp)";
    ss.newLine() << "{";
    ss.indent();

    ss.newLine() << ss.floatDecl("reachMaxM") << " = " << reachName << "_sample(h);";

    ss.newLine() << ss.floatDecl("focusJ") << " = " << ss.lerp("JMcusp.r", std::to_string(g.mid_J), std::string("min(1.f, 1.3f - (JMcusp.r / ") + std::to_string(g.limit_J_max)) << "));";
    ...
    const std::string & compressionName)
{
    ...
    ss.newLine() << ss.floatDecl("gamma_bottom") << " = 1.14f;";
    ...
    ss.newLine() << ss.float3Decl("reachBoundary") << " = " << getReachBoundaryName << "(JMboundary.r, JMboundary.g, h, JMcusp);";

    ss.newLine() << ss.floatDecl("difference") << " = max(1.0001f, reachBoundary.g / JMboundary.g);";
    ss.newLine() << ss.floatDecl("threshold") << " = max(0.75f, 1.f / difference);";
    ...
    const FixedFunctionOpData::Params & params)
{
    ...
    ss.newLine() << "if (Jx <= " << ss.lerp("JMcusp.r", std::to_string(g.limit_J_max), "0.3f") << ")";
    ss.newLine() << "{";
    ss.indent();
