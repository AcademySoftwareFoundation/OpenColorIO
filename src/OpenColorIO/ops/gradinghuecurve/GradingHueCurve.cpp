...
static const std::vector<GradingControlPoint> DefaultHueHueCtrl{
    {     0.0f,      0.0f},
    {1.f / 6.f, 1.f / 6.f},
    {2.f / 6.f, 2.f / 6.f},
    {     0.5f,      0.5f},
...
static const std::vector<GradingControlPoint> DefaultHueSatCtrl{
    {     0.0f, 1.0f},
    {1.f / 6.f, 1.0f},
    {2.f / 6.f, 1.0f},
    {     0.5f, 1.0f},
...
static const std::vector<GradingControlPoint> DefaultHueFxCtrl{
    {     0.0f, 0.0f},
    {1.f / 6.f,  0.f},
    {2.f / 6.f,  0.f},
    {     0.5f,  0.f},
...
static const std::vector<GradingControlPoint> DefaultLumSatCtrl{
    {0.0f, 1.0f},
    {0.5f, 1.0f},
    {1.0f, 1.0f}
};
static const std::vector<GradingControlPoint> DefaultLumSatLinCtrl{
    {-7.0f, 1.0f},
    {  0.f, 1.0f},
    { 7.0f, 1.0f}
};

static const std::vector<GradingControlPoint> DefaultSatSatCtrl{
    {0.0f, 0.0f},
    {0.5f, 0.5f},
    {1.0f, 1.0f}
};
static const std::vector<GradingControlPoint> DefaultSatLumCtrl{
    {0.0f, 1.0f},
    {0.5f, 1.0f},
    {1.0f, 1.0f}
};

static const std::vector<GradingControlPoint> DefaultLumLumCtrl{
    { 0.f,  0.f},
    {0.5f, 0.5f},
    {1.0f, 1.0f}
};
static const std::vector<GradingControlPoint> DefaultLumLumLinCtrl{
    {-7.0f, -7.0f},
    {  0.f,   0.f},
    { 7.0f,  7.0f}
};
...
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultHueHue(
    DefaultHueHueCtrl,
    BSplineType::HUE_HUE_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultHueSat(
    DefaultHueSatCtrl,
    BSplineType::PERIODIC_1_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultHueLum(
    DefaultHueSatCtrl,
    BSplineType::PERIODIC_1_B_SPLINE); // HUE_LUM use the same as HUE_SAT
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultHueFx(
    DefaultHueFxCtrl,
    BSplineType::PERIODIC_0_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultLumSat(
    DefaultLumSatCtrl,
    BSplineType::HORIZONTAL1_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultLumSatLin(
    DefaultLumSatLinCtrl,
    BSplineType::HORIZONTAL1_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultSatSat(
    DefaultSatSatCtrl,
    BSplineType::DIAGONAL_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultSatLum(
    DefaultSatLumCtrl,
    BSplineType::HORIZONTAL1_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultLumLum(
    DefaultLumLumCtrl,
    BSplineType::DIAGONAL_B_SPLINE);
const GradingBSplineCurveImpl GradingHueCurveImpl::DefaultLumLumLin(
    DefaultLumLumLinCtrl,
    BSplineType::DIAGONAL_B_SPLINE);

const std::array<
    std::reference_wrapper<const GradingBSplineCurveImpl>,
    static_cast<size_t>(HUE_NUM_CURVES)>
    GradingHueCurveImpl::DefaultCurvesLin(
        {std::ref(DefaultHueHue),
...
{
    return (c >= HUE_HUE && c < HUE_NUM_CURVES);
}
...
GradingHueCurveRcPtr GradingHueCurve::Create(
    ...
    ConstGradingBSplineCurveRcPtr hueFx)
{
    auto newCurve = std::make_shared<
        GradingHueCurveImpl>(hueHue, hueSat, hueLum, lumSat, satSat, lumLum, satLum, hueFx);
...
bool operator==(const GradingHueCurve & lhs, const GradingHueCurve & rhs)
{
    ...
    for (int c = 0; c < HUE_NUM_CURVES; ++c)
    {
        if (*(lhs.getCurve(static_cast<HueCurveType>(c)))
            != *(rhs.getCurve(static_cast<HueCurveType>(c))))
        {
            return false;
        }
    }
