...
public:
    ...
    void setDoubleValue(unsigned long index, double value) override { m_data[index] = (T)value; }

    double getDoubleValue(unsigned long index) override { return double(m_data[index]); }

    unsigned long getLength() const override { return m_length; }
    ...
    unsigned long getNumColorComponents() const override { return m_numColorComponents; }
    ...
    void adjustColorComponentNumber()
    {
        if (m_numColorComponents == 3)
        {
            ...
            {
                if (IsNan(m_data[idx * 3]) && IsNan(m_data[idx * 3 + 1])
    ...
    unsigned long getMaxColorComponents() const { return 3; }

    inline const Values & getValues() const { return m_data; }

    inline Values & getValues() { return m_data; }

    inline const T & operator[](unsigned long index) const { return m_data[index]; }

    inline T & operator[](unsigned long index) { return m_data[index]; }
    ...
    bool operator==(const ArrayT & a) const
    {
        if (this == &a)
            return true;
        return (m_length == a.m_length) && (m_numColorComponents == a.m_numColorComponents)
               && (m_data == a.m_data);
    }
