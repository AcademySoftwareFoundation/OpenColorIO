...
static int rsr_internal_I1D_refineSegment(float x, float * data, int low, int high)
{
    ...
    midPoint = (low + high) / 2;
    if (x < data[midPoint])
        return rsr_internal_I1D_refineSegment(x, data, low, midPoint);
...
float rsr_Interpolator1D_interpolate(float x, rsr_Interpolator1D * data)
{
    ...
    if (x > data->stims[data->nSamplePoints - 1])
...
rsr_Interpolator1D * rsr_Interpolator1D_createFromRaw(rsr_Interpolator1D_Raw * data)
{
    ...
    retval->parameters
        = (float *)malloc(5 * sizeof(float) * (data->length - 1)); // OCIO change: explicit cast
...
{
    unsigned int i;
    rsr_Interpolator1D_Raw * prelut = (rsr_Interpolator1D_Raw *)malloc(
        sizeof(rsr_Interpolator1D_Raw)); // OCIO change: explicit cast
    if (prelut == NULL)
        return NULL;
...
inline bool startswithU(const std::string & str, const std::string & prefix)
{
...
void LocalFileFormat::getFormatInfo(FormatInfoVec & formatInfoVec) const
{
    ...
    info.bake_capabilities
        = FormatBakeFlags(FORMAT_BAKE_CAPABILITY_3DLUT | FORMAT_BAKE_CAPABILITY_1D_3D_LUT);
    ...
    Interpolation interp) const
{
    ...
    {
        while (!startswithU(line, "END METADATA") || !istream)
        {
            nextline(istream, line);
    ...
    for (int c = 0; c < 3; ++c)
    {
        ...
        if (cpoints >= 2)
        {
            ...
            useprelut[c] = (!VecsEqualWithRelError(
                &(prelut_in[c][0]),
                static_cast<int>(prelut_in[c].size()),
                &(prelut_out[c][0]),
                static_cast<int>(prelut_out[c].size()),
                1e-6f));
        }
        else
        {
    ...
    if (csptype == "1D")
    {
        ...
        for (int i = 0; i < points1D; ++i)
        {
            ...
            if (!StringVecToFloatVec(floatArray, lineParts) || 3 != floatArray.size())
            {
                std::ostringstream os;
    ...
    else if (csptype == "3D")
    {
        ...
        if (!StringVecToIntVec(cubeSize, lineParts) || 3 != cubeSize.size())
        {
            std::ostringstream os;
        ...
        for (int i = 0; i < num3dentries; ++i)
        {
            ...
            const unsigned long arrayIdx
                = GetLut3DIndex_BlueFast(r, g, b, lutSize, lutSize, lutSize);
    ...
    if (useprelut[0] || useprelut[1] || useprelut[2])
    {
        ...
        for (int c = 0; c < 3; ++c)
        {
            ...
            rsr_Interpolator1D * interpolater = rsr_Interpolator1D_createFromRaw(cprelut_raw);
