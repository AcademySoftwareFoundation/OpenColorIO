...
private:
    ...
    static void CharacterDataHandler(void * userData, const XML_Char * s, int len)
    {
        ...
        else if (pImpl->m_data)
        {
            ...
            StringUtils::ReplaceInPlace(what, "'", "");
            StringUtils::ReplaceInPlace(what, "\n", "");
            // Append to lut string (cap at max possible: Max3DLUTLength^3 entries * 3 floats * 8
            // hex chars)
            if (pImpl->m_lutString.size() + what.size()
                > Max3DLUTLength * Max3DLUTLength * Max3DLUTLength * 3 * 8)
            {
