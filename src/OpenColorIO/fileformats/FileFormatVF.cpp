    ...
    Interpolation interp) const
{
    ...
    {
        ThrowErrorMessage("Expecting '#Inventor V2.1 ascii'.", fileName, lineNumber, line);
    }
...

    {
        ...
        while (nextline(istream, line))
        {
            ...
            if (!in3d)
            {
                ...
                {
                    if (parts.size() != 4 || !StringToInt(&size3d[0], parts[1].c_str())
                        || !StringToInt(&size3d[1], parts[2].c_str())
                        || !StringToInt(&size3d[2], parts[3].c_str()))
                    {
                        ThrowErrorMessage("Malformed grid_size tag.", fileName, lineNumber, line);
                    }
                    ...
                    if (size3d[0] != size3d[1] || size3d[0] != size3d[2])
                    {
                        ...
                        ThrowErrorMessage(os.str(), fileName, lineNumber, line);
                    }

                    if (size3d[0] < 2 || size3d[0] > static_cast<long>(Max3DLUTLength))
                    {
                        ThrowErrorMessage(
                            ("Grid size must be between 2 and " + std::to_string(Max3DLUTLength)
                             + ".")
                                .c_str(),
                            fileName,
                            lineNumber,
                ...
                else if (parts[0] == "global_transform")
                {
                    ...
                    if (!StringVecToFloatVec(global_transform, parts)
                        || global_transform.size() != 16)
                    {
                        ThrowErrorMessage(
                            "Malformed global_transform tag. "
            ...
            else // (in3d)
            {
                if (StringVecToFloatVec(tmpfloats, parts) && (tmpfloats.size() == 3))
                {
                    ...
                    {
                        ThrowErrorMessage("Too many 3D LUT entries.", fileName, lineNumber, line);
                    }
