...
void CTFReaderTransformElt::start(const char ** atts)
{
    ...
    while (atts[i])
    {
        ...
        else if (0 == Platform::Strcasecmp(ATTR_XMLNS, atts[i]))
        {
            ...
            {
                auto version
                    = CTFVersion(atts[i + 1], CTFVersion::StringFormat::VERSION_SMPTE_XMLNS);
        ...
        else if (0 == Platform::Strcasecmp(ATTR_COMP_CLF_VERSION, atts[i]))
        {
            ...
            try
            {
                ...
                requestedCLFVersion
                    = CTFVersion(verString, CTFVersion::StringFormat::VERSION_SMPTE_CLF);
            }
            catch (Exception & ce)
            {
                throwMessage(ce.what());
    ...
    if (!(isVersionFound || isCLFVersionFound || isSMPTEVersionFound))
    {
        ...
        {
            throwMessage(
                "No valid 'version', 'compCLFversion', or 'xmlns' attributes were found; "
...
bool IsRGBCurveType(const std::string & name)
{
        ...
        || 0 == Platform::Strcasecmp(TAG_RGB_CURVE_MASTER, name.c_str()))
    {
        ...
        0 == Platform::Strcasecmp(TAG_HUE_CURVE_HUE_HUE, name.c_str())
...
void CTFReaderReferenceElt::start(const char ** atts)
{
    ...
    if (!alias.empty())
    {
        ...
        {
            throwMessage(
                "alias & path attributes for Reference should not be "
                "both defined.");
        }
        ...
        {
            throwMessage(
                "alias & basepath attributes for Reference should not be "
                "both defined.");
        }
