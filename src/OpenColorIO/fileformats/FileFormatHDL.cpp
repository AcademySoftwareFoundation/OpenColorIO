    ...
    while (istream >> word)
    {
        ...
        else if (inlut)
        {
            ...
            if (result.ec == std::errc())
            {
                ...
                if (lutValues[lutname].size()
                    > Max3DLUTLength * Max3DLUTLength * Max3DLUTLength * 3)
                {
                    std::ostringstream os;
    ...
    Interpolation interp) const
{
...

    {
        ...
        if (cachedFile->hdltype == "3d" || cachedFile->hdltype == "3d+1d")
        {
            ...
            {
                std::ostringstream os;
                os << "3D LUT cube size must be between 2 and " << Max3DLUTLength
                   << ", found: " << size_3d;
                throw Exception(os.str().c_str());
            }
        ...
        if (cachedFile->hdltype == "c")
        {
            ...
            {
                std::ostringstream os;
                os << "1D LUT size must be between 2 and " << Max1DLUTLength
                   << ", found: " << size_1d;
                throw Exception(os.str().c_str());
            }
        }
        ...
        if (cachedFile->hdltype == "3d+1d")
        {
            ...
            {
                std::ostringstream os;
                os << "Prelut size must be between 2 and " << Max1DLUTLength
                   << ", found: " << size_prelut;
                throw Exception(os.str().c_str());
            }
        }
