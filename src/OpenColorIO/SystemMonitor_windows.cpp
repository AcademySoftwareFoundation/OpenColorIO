            ...
            && (dispDevice.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP))
        {
            ...
            if (hDC)
            {
                ...
                const std::tstring extra = friendlyNameExists
                                               ? friendlyMonitorNames.at(dispNum)
                                               : std::tstring(dispDevice.DeviceString);
                ...
                if (GetICMProfile(hDC, &pathLength, icmPath))
                {
                    ...
                    m_monitors.push_back(
                        {Platform::Utf16ToUtf8(displayName), Platform::Utf16ToUtf8(icmPath)});
#else
                ...
                else
                {
                    ...
                    oss << TEXT("Unable to access the ICM profile for the monitor '") << displayName
                        << TEXT("'.");
