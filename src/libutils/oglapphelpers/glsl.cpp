...
void OpenGLBuilder::Uniform::use()
{
    ...
    else if (m_data.m_vectorFloat.m_getSize && m_data.m_vectorFloat.m_getVector)
    {
        glUniform1fv(
            m_handle,
            (GLsizei)m_data.m_vectorFloat.m_getSize(),
    ...
    else if (m_data.m_vectorInt.m_getSize && m_data.m_vectorInt.m_getVector)
    {
        glUniform1iv(
            m_handle,
            (GLsizei)m_data.m_vectorInt.m_getSize(),
...
void OpenGLBuilder::useAllTextures()
{
    ...
    for (size_t idx = 0; idx < max; ++idx)
    {
        ...
        glUniform1i(
            glGetUniformLocation(m_program, data.m_samplerName.c_str()),
...
unsigned OpenGLBuilder::GetTextureMaxWidth()
{
    ...
    while (w > 1)
    {
        glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_RGB32F_ARB, w, h, 0, GL_RGB, GL_FLOAT, NULL);
