    ...
    , m_viewportHeight{winHeight}
{
    ...
    RECT windowRect
        = {0, 0, static_cast<LONG>(m_viewportWidth), static_cast<LONG>(m_viewportHeight)};
...
void DxApp::setShader(GpuShaderDescRcPtr & shaderDesc)
{
    ...
    {
        const UINT requiredSRVs = 1 + shaderDesc->getNumTextures() + shaderDesc->getNum3DTextures();
        if (requiredSRVs > CbvSrvHeapSize)
        {
            std::ostringstream oss;
            oss << "DxApp: shader needs " << requiredSRVs
                << " SRV descriptors but the CBV/SRV heap only has " << CbvSrvHeapSize << " slots.";
    ...
    m_hlslBuilder
        = HLSLBuilder::Create(shaderDesc, m_device.Get(), m_commandList.Get(), m_cbvSrvHeap.Get());
    ...
    // root signature can unconditionally declare a CBV at b0.
    {
        ...
        ThrowIfFailed(
            m_constantBuffer->Map(0, &readRange, reinterpret_cast<void **>(&m_cbMappedData)));
    ...
    ThrowIfFailed(m_dxcUtils->CreateBlobFromPinned(
        fullShader.c_str(),
        static_cast<UINT32>(fullShader.size()),
        DXC_CP_UTF8,
        &sourceBlob));
    ...
    ThrowIfFailed(
        m_dxcCompiler
            ->Compile(&sourceBuffer, psArgs, _countof(psArgs), nullptr, IID_PPV_ARGS(&psResult)));
    HRESULT psHr;
...
void DxApp::reshape(int width, int height)
{
    ...
    if (!m_floatRtvHeap)
    {
        ...
        ThrowIfFailed(
            m_device->CreateDescriptorHeap(&floatRtvHeapDesc, IID_PPV_ARGS(&m_floatRtvHeap)));
    }
            ...
            for (unsigned i = 0; i < numUniforms; ++i)
            {
                ...
                switch (data.m_type)
                {
                    ...
                    case UNIFORM_FLOAT3:
                        ...
                        break;
                    // HLSL constant-buffer array entries (float or int) occupy a full float4
                    // (16-byte) slot.
...
void DxApp::ensureVertexShaderCompiled()
{
    ...
    ThrowIfFailed(
        m_dxcCompiler
            ->Compile(&vsBuffer, vsArgs, _countof(vsArgs), nullptr, IID_PPV_ARGS(&vsResult)));
    HRESULT vsHr;
